...
describe('buildChatGenerationMessages', () => {
  it('projects the selected branch, preserves reasoning and exact tool arguments, and excludes the active assistant', () => {
    ...
    const tool: MessageNode = { id: toMessageId({ raw: 'tool-result' }), role: 'tool', createdAt: 3, modelId: undefined, lmParameters: undefined, parts: [{ type: 'tool_result', result: { toolCallId: callId, status: 'success', content: { type: 'text', text: '20 C' } } }], replies: { items: [followUp] } };
    const args = `\
{
  "city": "Tokyo",
  "unit": "C"
    ...
    generated.parts = [
      { type: 'reasoning', text: '  東京を調べる。\n', completeness: 'complete' },
      { type: 'text', text: 'calling', completeness: 'complete' },
      { type: 'tool_call', toolCall: { id: callId, type: 'function', function: { name: 'lookup_weather', arguments: args } } },
    ];
    ...
    expect(messages).toEqual([
      { id: toMessageId({ raw: 'system_prompt_0' }), role: 'system', parts: [{ type: 'text', text: 'system', completeness: 'complete' }] },
  ...
  it('preserves multimodal user parts as references rather than choosing an API image_url representation', () => {
    ...
    node.parts.push({ type: 'attachment', attachment: { id: toAttachmentId({ raw: 'att' }), binaryObjectId: toBinaryObjectId({ raw: 'image' }), originalName: 'image.png', mimeType: 'image/png', size: 10, uploadedAt: 1, status: 'persisted' } });
  ...
  it('does not turn interruption into model content, nor strip literal think tags from partial history', () => {
    ...
    node.parts.push({ type: 'text', text: '<think>原文</think>途中', completeness: 'partial' });
  ...
  it('keeps deterministic system IDs without colliding with stored messages', () => {
    ...
    expect(first[2]?.parts).toEqual([{ type: 'text', text: '', completeness: 'complete' }]);
  });
