...
describe('OPFSStorageProvider Directory Isolation', () => {
  ...
  it('should create and use "llm-web-ui-storage" directory within OPFS root', async () => {
    ...
    await provider.init();
    expect(mockOpfsRoot.entries.has('llm-web-ui-storage')).toBe(true);
  ...
  it('should only clear contents within the "llm-web-ui-storage" directory', async () => {
    const provider = new OPFSStorageProvider();
    await provider.init();
    const storageDir = mockOpfsRoot.entries.get('llm-web-ui-storage') as MockFileSystemDirectoryHandle;
