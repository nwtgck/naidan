...
type AsyncString<Message> = Message extends () => infer Result
...
export type AsyncStrings = {
  readonly [Key in keyof Strings]: AsyncString<Strings[Key]>;
};

export const stringsAsync = new Proxy({}, {
  get(_target, property) {
    if (typeof property !== 'string') {
      return undefined;
    }
