...
const parameters = ref(defaultAudioParameters());
...
const canGenerate = computed(() => !blocked.value && runtimeReady.value && models.value.some(entry => entry.id === model.value) && text.value.trim().length > 0);
let disposed = false;
let generation = 0;
let unsubscribe: (() => void) | undefined;

// Labels use language self-names; the selected language is not inferred from text.
const languages = [
  { value: 'en', label: 'English' }, { value: 'ja', label: '日本語' }, { value: 'zh', label: '中文' },
  { value: 'de', label: 'Deutsch' }, { value: 'it', label: 'Italiano' }, { value: 'pt', label: 'Português' },
  { value: 'es', label: 'Español' }, { value: 'ko', label: '한국어' }, { value: 'fr', label: 'Français' }, { value: 'ru', label: 'Русский' },
];
...
function chooseReference({ event }: { event: Event }): void {
  ...
  invalid.value = false; failure.value = undefined;
}
...
async function generate(): Promise<void> {
  ...
  failure.value = undefined; invalid.value = false; stopped.value = false;
    ...
    <form @submit.prevent="generate" tw-class="space-y-5">
      <fieldset :disabled="blocked" tw-class="space-y-5 disabled:opacity-60">
        <div tw-class="space-y-2">
          ...
          <select :id="`${id}-model`" v-model="model" required data-testid="audio-model" tw-class="w-full rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 text-sm">
        ...
        <div tw-class="space-y-2">
          ...
          <textarea :id="`${id}-text`" v-model="text" required maxlength="8192" rows="5" :placeholder="lazyStrings.audioGeneration__input_placeholder()" data-testid="audio-text" tw-class="w-full resize-y rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 text-sm leading-relaxed" />
        </div>
        <div tw-class="space-y-2">
          <label :for="`${id}-language`" tw-class="block text-sm font-medium">{{ lazyStrings.audioGeneration__language() }}</label>
          <select :id="`${id}-language`" v-model="parameters.language" :aria-describedby="`${id}-language-help`" data-testid="audio-language" tw-class="w-full sm:w-64 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 text-sm">
        ...
        <div tw-class="space-y-2">
          ...
          <input :id="`${id}-reference`" ref="referenceInput" type="file" accept=".wav,.mp3,.flac,audio/wav,audio/mpeg,audio/flac" :aria-describedby="`${id}-reference-help`" data-testid="audio-reference" tw-class="block w-full text-sm" @change="chooseReference({ event: $event })" />
        ...
        <details tw-class="rounded-xl border border-gray-200 dark:border-gray-700 p-4">
          ...
          <div tw-class="pt-4 space-y-4">
            <div tw-class="space-y-2">
              ...
              <select :id="`${id}-backend`" v-model="parameters.audioBackend" data-testid="audio-backend" tw-class="w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-2 text-sm">
            ...
            <div tw-class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <label tw-class="space-y-1 text-sm"><span>{{ lazyStrings.audioGeneration__context_tokens() }}</span><input v-model.number="parameters.contextTokens" type="number" min="1024" max="8192" step="1" required data-testid="audio-context" tw-class="block w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-2" /></label>
              <label tw-class="space-y-1 text-sm"><span>{{ lazyStrings.audioGeneration__maximum_steps() }}</span><input v-model.number="parameters.maxFrames" type="number" min="1" max="2048" step="1" required data-testid="audio-max-frames" tw-class="block w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-2" /></label>
              <label tw-class="space-y-1 text-sm"><span>{{ lazyStrings.audioGeneration__backbone_temperature() }}</span><input v-model.number="parameters.temperature" type="number" min="0" max="2" step="any" required tw-class="block w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-2" /></label>
              <label tw-class="space-y-1 text-sm"><span>{{ lazyStrings.audioGeneration__backbone_top_k() }}</span><input v-model.number="parameters.topK" type="number" min="1" max="256" step="1" required tw-class="block w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-2" /></label>
              <label tw-class="space-y-1 text-sm"><span>{{ lazyStrings.audioGeneration__backbone_top_p() }}</span><input v-model.number="parameters.topP" type="number" min="0.001" max="1" step="any" required tw-class="block w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-2" /></label>
              <label tw-class="space-y-1 text-sm"><span>{{ lazyStrings.audioGeneration__seed() }}</span><input v-model.number="parameters.seed" type="number" min="0" max="4294967295" step="1" required tw-class="block w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-2" /></label>
            </div>
      ...
      <div v-if="invalid || failure" role="alert" data-testid="audio-error" tw-class="rounded-xl border border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-900/10 p-4 text-sm text-red-700 dark:text-red-400"><p>{{ errorMessage }}</p><code v-if="failure" tw-class="mt-2 block text-xs">{{ failure }}</code></div>
    </form>
