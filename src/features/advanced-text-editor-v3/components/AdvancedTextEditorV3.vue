...
import { createAdvancedTextEditorV3WorkerClient } from '@/features/advanced-text-editor-v3/worker/client';
import type {
  AdvancedTextEditorV3Match,
  AdvancedTextEditorV3WorkerClient,
} from '@/features/advanced-text-editor-v3/worker/types';
...
<template>
  ...
  >
    ...
    >
      ...
      <div tw-class="flex-1 flex flex-col min-w-0 bg-transparent relative">
            ...
            <div tw-class="flex items-center gap-3">
              <div tw-class="relative flex-1 group">
                ...
                <div v-if="findText" tw-class="absolute right-2 top-1.5 flex items-center gap-2 bg-gray-50 dark:bg-white/10 px-2 py-1 rounded-lg border border-gray-200 dark:border-white/10">
                  <span
                    v-if="isSearchBusy"
                    tw-class="text-[10px] text-blue-500 font-mono font-bold"
                  ...
                  <div tw-class="flex items-center">
                    <button @click="prevMatch" :disabled="isSearchBusy" tw-class="p-0.5 hover:bg-gray-200 dark:hover:bg-white/10 rounded transition-colors disabled:opacity-40"><ArrowUpIcon tw-class="w-3.5 h-3.5 text-gray-400" /></button>
                    <button @click="nextMatch" :disabled="isSearchBusy" tw-class="p-0.5 hover:bg-gray-200 dark:hover:bg-white/10 rounded transition-colors disabled:opacity-40"><ArrowDownIcon tw-class="w-3.5 h-3.5 text-gray-400" /></button>
                  </div>
                </div>
              </div>
            ...
            <div tw-class="flex items-center gap-3">
              ...
              <div tw-class="flex items-center gap-2">
                <button
                  ...
                  tw-class="px-4 py-2 text-[11px] font-bold uppercase tracking-wider bg-white dark:bg-white/10 hover:bg-gray-50 dark:hover:bg-white/20 text-gray-600 dark:text-gray-200 rounded-xl transition-all border border-gray-200 dark:border-white/10 shadow-sm disabled:opacity-40"
                ...
                <button
                  ...
                  :disabled="isReplaceBusy"
                  tw-class="px-4 py-2 text-[11px] font-bold uppercase tracking-wider bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-all shadow-lg shadow-blue-500/20 disabled:opacity-40"
        ...
        <div tw-class="flex-1 flex flex-col min-h-0 bg-transparent relative group/editor">
          ...
          <textarea
            ...
            @input="handleInput"
            @scroll="handleScroll"
            :tw-class="['w-full overscroll-contain h-full pr-16 py-5 font-mono text-sm bg-transparent outline-none resize-none text-gray-800 dark:text-gray-200 leading-[21px] selection:bg-blue-500/45 selection:text-white z-10 transition-all duration-200', [
              wrapMode === 'wrap-off'
          ...
          <div v-if="lines.length <= 1 && !lines[0]" tw-class="absolute inset-0 flex items-center justify-center pointer-events-none opacity-[0.02] dark:opacity-[0.03]">
          ...
          <div v-if="isMultiEditMode" tw-class="absolute bottom-12 left-1/2 -translate-x-1/2 w-96 z-50 bg-white dark:bg-slate-900 border border-amber-500/30 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] p-4 ring-1 ring-black/5">
            ...
            <div tw-class="relative group">
              ...
              <div tw-class="absolute right-3 top-2.5 flex items-center gap-1">
                <button @click="applyMultiEdit" :disabled="isMultiEditBusy" tw-class="p-1 hover:bg-gray-100 dark:hover:bg-white/10 rounded-lg text-gray-400 hover:text-green-500 transition-colors disabled:opacity-40" :title="lazyStrings.advancedTextEditor__confirm_enter()">
            ...
            <div tw-class="mt-2 text-[9px] text-gray-400 text-center font-medium">
              ...
              <span v-else>{{ lazyStrings.advancedTextEditor__type_to_rename_all() }} <span tw-class="text-gray-500">{{ lazyStrings.advancedTextEditor__enter() }}</span> {{ lazyStrings.advancedTextEditor__to_apply() }} <span tw-class="text-gray-500">{{ lazyStrings.advancedTextEditor__esc() }}</span> {{ lazyStrings.advancedTextEditor__to_cancel() }}</span>
            </div>
          </div>
        </Transition>
      ...
      <!-- Sidebar (Right) -->
      <div tw-class="w-12 border-l border-gray-100 dark:border-white/10 flex flex-col items-center py-4 gap-4 bg-gray-50 dark:bg-slate-950/50 z-30 shrink-0">
        <button
          @click="handleClose"
        ...
        <div tw-class="flex flex-col gap-1">
          <button
            @click="toggleMode"
            :tw-class="['p-2.5 rounded-xl transition-all hover:shadow-sm group', localMode === 'textarea' ? 'bg-orange-500/20 text-orange-400' : 'hover:bg-white dark:hover:bg-white/5 text-gray-500']"
          ...
          >
            <AlignLeftIcon tw-class="w-4.5 h-4.5" />
          </button>
          <button
