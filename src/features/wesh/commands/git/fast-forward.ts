...
export async function fastForwardHead({ files, repository, targetObjectId, reflog, contentConfig }: {
  files: GitFiles,
  ...
  const currentCommit = await readCommit({ files, repository, objectId: head.objectId });
  const targetCommit = await readCommit({ files, repository, objectId: targetObjectId });
  const currentHeadEntries = await readTreeIntoIndex({ files, repository, treeObjectId: currentCommit.treeObjectId });
  const currentIndexEntries = await readIndex({ files, repository });
  const targetEntries = await readTreeIntoIndex({ files, repository, treeObjectId: targetCommit.treeObjectId });
