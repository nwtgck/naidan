...
export function createProductionReplayTestWorkerConstructor({ scriptUrl, start, onConstructed }: {
  scriptUrl: URL,
  start: ({ worker }: { worker: ProductionReplayTestWorker }) => Promise<void>,
  onConstructed: ({ worker }: { worker: ProductionReplayTestWorker }) => void,
}) {
  let constructed = false;
  return class extends ProductionReplayTestWorker {
...
export class ProductionReplayTestWorker extends EventTarget {
  ...
  constructor({ start }: { start: ({ worker }: { worker: ProductionReplayTestWorker }) => Promise<void> }) {
    super();
