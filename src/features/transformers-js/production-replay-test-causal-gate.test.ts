...
function recording() {
  return parseProductionReplayTextEvidence({ value: {
    schemaVersion: 1,
...
describe('Captured text causal gate mechanics', () => {
  ...
  it('does not turn a prefix into an unsupported complete capture', () => {
    const evidence = recording();
    expect(() => parseProductionReplayTextEvidence({ value: {
  ...
  it('rejects an ending labeled below budget when its count actually reaches that budget', () => {
    ...
    expect(() => parseProductionReplayTextEvidence({ value: evidence })).toThrow('ending must precede request budget');
  });
  ...
  it('does not relabel the shorter recorded output as a budget-length prefix', () => {
    ...
    expect(() => parseProductionReplayTextEvidence({ value: evidence })).toThrow('recorded ending scope');
  });
});
