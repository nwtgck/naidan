...
import { createProductionReplayTestWorkerConstructor, type ProductionReplayTestWorker } from './production-replay-test-transport';
...
import type { createProductionReplayTestImagePlatform } from './production-replay-test-image-platform';

type Runtime = typeof import('@huggingface/transformers');
export type ProductionReplayGenerate = ({ options, model, tokenizer, runtime }: {
 ...
 */
export async function createProductionReplayTestRuntime({ modelId, expectedRevision, artifacts, generate, imagePlatform }: {
  modelId: string,
  expectedRevision: string,
  artifacts: ReadonlyArray<{ path: string, bytes: Uint8Array }>,
  generate: ProductionReplayGenerate,
  imagePlatform: {
    platform: ReturnType<typeof createProductionReplayTestImagePlatform>,
  ...
  const seeded = new Map(fixture.files);
  ...
  for (const [path, bytes] of seeded) {
    const key = `models/huggingface.co/${modelId}/resolve/${expectedRevision}/${path}`;
  ...
  try {
    ...
    const workers: ProductionReplayTestWorker[] = [];
    ...
    setOwnedGlobal({ key: 'Worker', value: createProductionReplayTestWorkerConstructor({
