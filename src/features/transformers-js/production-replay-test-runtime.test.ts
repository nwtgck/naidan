...
import { createProductionReplayTestRuntime } from './production-replay-test-runtime';
import { createProductionReplayTestWorkerConstructor } from './production-replay-test-transport';
...
import type { ProductionReplayGenerate } from './production-replay-test-runtime';
import type { LmProvider } from '@/01-models/lm';
import { createProductionProviderTrace } from './model-support-investigation/logic/production-provider-trace';
import type { CaptureRequestInput } from './model-support-investigation/logic/production-provider-capture-plan';
...
function mechanicsArguments({ generate }: { generate: ProductionReplayGenerate }) {
  return {
...
describe('Production replay explicit image platform ownership', () => {
  ...
  it('does not enable local data transport when the image platform option is absent', async () => {
    ...
    const harness = await createProductionReplayTestRuntime(mechanicsArguments({ generate: async () => {
  ...
  it('rejects a non-data image capability before changing globals or importing the runtime', async () => {
    ...
    try {
      await expect(createProductionReplayTestRuntime({
        ...mechanicsArguments({ generate: async () => {
          throw new Error('No inference in invalid capability test');
        } }),
        imagePlatform: { platform: createProductionReplayTestImagePlatform(), allowedDataUrls: ['http://localhost/synthetic.png'] },
  ...
  it('restores image globals even when service unload rejects', async () => {
    ...
    const harness = await createProductionReplayTestRuntime({
      ...
      imagePlatform: { platform: createProductionReplayTestImagePlatform(), allowedDataUrls: [dataUrl] },
    });
...
describe('Production replay construction ownership', () => {
  it('keeps the setup error and restores parent globals even when a spy restoration throws', async () => {
    ...
    try {
      await expect(createProductionReplayTestRuntime(mechanicsArguments({ generate: async () => {
...
describe('Production replay native inference boundary', () => {
  it('rejects a same-shape attention mask before calling the injected inference callback', async () => {
    let invocation: Parameters<ProductionReplayGenerate>[0] | undefined;
    const generate = vi.fn<ProductionReplayGenerate>(async value => {
      invocation = value;
      throw new Error('Synthetic inference inspection');
    });
    const harness = await createProductionReplayTestRuntime(mechanicsArguments({ generate }));
    try {
...
describe('Production replay Worker construction boundary', () => {
  it('rejects a foreign first entry before creating channels or scheduling Production startup', async () => {
    ...
    const Worker = createProductionReplayTestWorkerConstructor({ scriptUrl, start, onConstructed });
...
describe('Production replay runtime mechanics, not captured model semantics', () => {
  ...
  it('preserves real Comlink settlement and immediate next inputs inside the investigation Provider owner', async () => {
    ...
    function createNativeBoundary() {
      ...
      const generate: ProductionReplayGenerate = async ({ options, tokenizer, runtime }) => {
        const input = options.input_ids;
    ...
    const baseline = await createProductionReplayTestRuntime(mechanicsArguments({ generate: baselineNative.generate }));
    ...
    const captured = await createProductionReplayTestRuntime(mechanicsArguments({ generate: capturedNative.generate }));
    ...
    const { createProductionProviderCaptureOwner } = await import('./model-support-investigation/logic/production-provider-capture-owner');
    const { createTransformersJsGenerationCaptureClient } = await import('./worker/client-hosted');
  ...
  it('reaches injected inference only after actual Provider, Comlink, offline Load and tokenizer', async () => {
    const generate = vi.fn(async ({ options, tokenizer }: Parameters<import('./production-replay-test-runtime').ProductionReplayGenerate>[0]) => {
