...
import evidenceJson from './production-replay-smollm2-1.7b.evidence.json';
import continuityJson from './production-replay-smollm2-1.7b.continuity.evidence.json';
import inputJson from './production-replay-smollm2-1.7b.input.evidence.json';
import toolInputJson from './production-replay-smollm2-1.7b.tool-input.evidence.json';
...
describe('SmolLM2 1.7B captured Production Provider replay', () => {
  it('preserves the recorded first user input without supplying any output tokens', async () => {
    ...
    const harness = await createProductionReplayTestRuntime({
      modelId: 'HuggingFaceTB/SmolLM2-1.7B-Instruct', expectedRevision: source.identity.resolvedRevision, imagePlatform: undefined,
...
describe('SmolLM2 1.7B original system and history input contracts', () => {
...
describe('SmolLM2 1.7B captured Production Provider replay', () => {
  it('delivers the recorded first-turn stream exactly before Provider settlement', async () => {
    ...
    const harness = await createProductionReplayTestRuntime({
      ...
      modelId: 'HuggingFaceTB/SmolLM2-1.7B-Instruct', expectedRevision: evidence.identity.resolvedRevision,
  ...
  it('preserves the recorded end-of-turn output with explicitly supplied assistant history', async () => {
    ...
    const harness = await createProductionReplayTestRuntime({
      imagePlatform: undefined,
      modelId: 'HuggingFaceTB/SmolLM2-1.7B-Instruct', expectedRevision: continuity.identity.resolvedRevision,
  ...
  it('uses only the settled first callback text for a second request in the same loaded runtime', async () => {
    ...
    const harness = await createProductionReplayTestRuntime({
      modelId: 'HuggingFaceTB/SmolLM2-1.7B-Instruct', expectedRevision: evidence.identity.resolvedRevision, imagePlatform: undefined,
      artifacts: [{ path: 'onnx/model_q4f16.onnx', bytes: createSyntheticModelBody({ modelId: inputEvidence.modelId, revision: inputEvidence.revision, path: 'onnx/model_q4f16.onnx' }) }],
...
describe('SmolLM2 1.7B native tool-input limitations through public Provider', () => {
