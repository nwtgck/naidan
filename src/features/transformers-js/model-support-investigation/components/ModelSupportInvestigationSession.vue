...
import type { InvestigationSessionSnapshot, InvestigationSessionView } from "@/features/transformers-js/model-support-investigation/logic/investigation-session";
...
const emit = defineEmits<{
  (event: "close"): void,
  (event: "newInvestigation", replacement: { viewId: string; snapshot: Extract<InvestigationSessionSnapshot, { view: 'setup' }> }): void,
}>();
...
const newInvestigationPending = ref(false);
...
function close(): void {
  if (running.value || newInvestigationPending.value) return;
  retireCurrentView();
  emit("close");
}
...
type SessionWorkerClient = ReturnType<typeof createModelSupportInvestigationWorkerClient>
...
function retireCurrentView(): void {
  ...
  // starts physical Worker disposal now; a reopened view inherits the barrier.
  void sessionView.retire({ dispose: disposeSessionWorkers }).then(result => {
    switch (result.status) {
...
async function newInvestigation(): Promise<void> {
  if (!started.value || !sessionReady.value || running.value || evidenceExporting.value || newInvestigationPending.value || !sessionView.isActive()) return;
  newInvestigationPending.value = true;
  try {
    const title = await ensureStrings.ModelSupportInvestigationModal__new_investigation();
    const message = await ensureStrings.ModelSupportInvestigationModal__replace_results_and_return_to_setup();
    if (!sessionView.isActive() || running.value || evidenceExporting.value) return;
    const confirmed = await showConfirm({ title, message, confirmButtonText: title });
    if (!confirmed || !sessionView.isActive() || running.value || evidenceExporting.value) return;
    const snapshot: Extract<InvestigationSessionSnapshot, { view: 'setup' }> = {
      view: 'setup',
    ...
    const retirement = await sessionView.retire({ dispose: disposeSessionWorkers });
    switch (retirement.status) {
    case 'failed': teardownError.value = retirement.error; return;
    case 'complete': break;
    default: {
      const _ex: never = retirement;
      return _ex;
    }
    }
    if (!componentDisposed) emit('newInvestigation', { viewId: sessionView.viewId, snapshot });
  } finally {
    newInvestigationPending.value = false;
  }
}

async function startInvestigation(): Promise<void> {
  if (started.value || !sessionReady.value || running.value || evidenceExporting.value || !sessionView.isActive() || !hasRequestedScope.value || !commitTargetDraft()) return;
  ...
  >
    ...
    >
      <header tw-class="px-6 py-5 bg-gray-50 dark:bg-gray-800/70 border-b border-gray-200 dark:border-gray-700 flex items-start justify-between gap-4">
        ...
        <button
          type="button"
          :disabled="running || newInvestigationPending"
      ...
      >
        <section tw-class="space-y-4">
          ...
          <div v-if="run" tw-class="rounded-xl border border-gray-200 dark:border-gray-700 p-3 space-y-1">
            ...
            <p v-if="run.freshMetadata" tw-class="text-xs font-mono text-gray-600 dark:text-gray-400" data-testid="model-support-fresh-metadata-bytes">
              {{ run.freshMetadata.receivedBytes }} / {{ run.freshMetadata.maximumBytes }} bytes
            </p>
            <p tw-class="text-xs text-gray-500 dark:text-gray-400" data-testid="model-support-fresh-metadata-limit">
              {{ lazyStrings.ModelSupportInvestigationSession__metadata_preparation_does_not_verify_full_download_or_load() }}
            </p>
          </div>
      ...
      <footer tw-class="px-6 py-4 bg-gray-50 dark:bg-gray-800/70 border-t border-gray-200 dark:border-gray-700 flex flex-wrap items-center justify-end gap-3">
        <button
          v-if="!started"
          type="button"
        ...
        <button
          v-if="started"
          type="button"
          :disabled="!sessionReady || teardownError !== undefined || running || evidenceExporting || newInvestigationPending"
          tw-class="px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-xs font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2"
          data-testid="model-support-investigation-new"
          @click="newInvestigation"
        >
          <PlusIcon tw-class="w-4 h-4" />
          {{ lazyStrings.ModelSupportInvestigationModal__new_investigation() }}
        </button>
        <button
