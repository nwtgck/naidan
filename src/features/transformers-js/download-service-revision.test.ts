...
it('does not leave a failed new-revision Download hint that would replace a usable offline revision', async () => {
  ...
  expect(boundary.prepare).toHaveBeenCalledExactlyOnceWith({ modelId, revision: advancedRevision, progressCallback: expect.any(Function), onDownloadProgress: expect.any(Function) });
  expect(boundary.verifyRevision).not.toHaveBeenCalled();
