...
import evidenceJson from './production-replay-smollm2-135m.evidence.json';
import continuityJson from './production-replay-smollm2-135m.continuity.evidence.json';
import inputJson from './production-replay-smollm2-135m.input.evidence.json';
import toolInputJson from './production-replay-smollm2-135m.tool-input.evidence.json';
...
describe('SmolLM2 135M captured Production Provider replay', () => {
  it('preserves the recorded first user input without supplying any output tokens', async () => {
    ...
    const harness = await createProductionReplayTestRuntime({
      modelId: 'HuggingFaceTB/SmolLM2-135M-Instruct', expectedRevision: source.identity.resolvedRevision, imagePlatform: undefined,
...
describe('SmolLM2 135M original system and history input contracts', () => {
...
describe('SmolLM2 135M captured Production Provider replay', () => {
  it('delivers the recorded no-tools prefix before the public Provider settles', async () => {
    ...
    const harness = await createProductionReplayTestRuntime({
      ...
      modelId: 'HuggingFaceTB/SmolLM2-135M-Instruct', expectedRevision: evidence.identity.resolvedRevision,
...
I hope this message finds you well.`,
  ...
  it('preserves the recorded follow-up prefix with an explicitly supplied assistant history', async () => {
    ...
    const harness = await createProductionReplayTestRuntime({
      imagePlatform: undefined,
      modelId: 'HuggingFaceTB/SmolLM2-135M-Instruct', expectedRevision: continuity.identity.resolvedRevision,
      artifacts: [{ path: 'onnx/model_q4f16.onnx', bytes: Uint8Array.of(1) }],
      generate: async ({ options, tokenizer, runtime }) => {
  ...
  it('uses only the settled first callback text for a second request in the same loaded runtime', async () => {
    ...
    const harness = await createProductionReplayTestRuntime({
      modelId: 'HuggingFaceTB/SmolLM2-135M-Instruct', expectedRevision: evidence.identity.resolvedRevision, imagePlatform: undefined,
      artifacts: [{ path: 'onnx/model_q4f16.onnx', bytes: createSyntheticModelBody({ modelId: inputEvidence.modelId, revision: inputEvidence.revision, path: 'onnx/model_q4f16.onnx' }) }],
...
describe('SmolLM2 135M native tool-input limitations through public Provider', () => {
  it.each(toolInputEvidence.cases)('$caseId preserves arguments up to a template that does not render tool definitions or calls', async scenario => {
    ...
    let actualTokenizer: Parameters<ProductionReplayGenerate>[0]['tokenizer'] | undefined;
