...
import { historicalRepositoryPath } from './model-historical-evidence-paths';
...
function configFromEvidence({ name }: { name: string }): Record<string, unknown> {
  const fixture = JSON.parse(readFileSync(historicalRepositoryPath({ name }), 'utf8')) as { modelType: string; architectures: string[]; transformersJsConfig: Record<string, unknown> };
  return {
