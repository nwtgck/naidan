...
import { createProviderReplayTestImagePlatform, TEST_ONLY } from './provider-replay-test-image-platform';
...
describe('opaque solid full-frame image platform', () => {
  ...
  it('rejects a closed bitmap and foreign platform objects', async () => {
    const platform = createProviderReplayTestImagePlatform();
    const other = createProviderReplayTestImagePlatform();
    const bitmap = await platform.createImageBitmap(new Blob([originalPng], { type: 'image/png' }));
    bitmap.close();
  ...
  it('does not install or change caller-owned globals', () => {
    ...
    createProviderReplayTestImagePlatform();
