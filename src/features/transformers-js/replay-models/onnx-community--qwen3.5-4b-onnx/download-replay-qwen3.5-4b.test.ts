...
describe('Qwen3.5 4B Download replay', () => {
  describe('metadata', () => {
  ...
  describe('new Download and fresh Offline Load', () => {
    it('exact Download preserves its selected artifacts through fresh offline Load', async () => {
      ...
      const expectedArtifacts = [
        ...
        { path: 'onnx/vision_encoder_q4f16.onnx_data', size: 198159360 },
      ];
      // The native multimodal config selects decoder/embed/vision for new Download.
      const h = await connectRawDownload({ modelId, revision, remoteRefs: new Map() });
      try {
  ...
  describe('candidate availability', () => {
    it('excludes a candidate with missing onnx/decoder_model_merged_q4f16.onnx_data_1 before ORT without repairing it', async () => {
      const modelId = 'onnx-community/Qwen3.5-4B-ONNX';
      const revision = '74d8caba2117fd5f41d655e9cc27eda1338662b3';
