...
describe('LFM2.5 350M Download replay', () => {
  describe('metadata', () => {
  ...
  describe('candidate availability', () => {
    it('excludes a candidate with missing onnx/model_q4f16.onnx_data before ORT without repairing it', async () => {
      const modelId = 'LiquidAI/LFM2.5-350M-ONNX';
      const revision = 'd11593fd9eb408e322667926656598896c2d5ff9';
  ...
  describe('post-plan invariants', () => {
    it('stops when required external data disappears after complete planning instead of falling back to complete q4', async () => {
      const modelId = 'LiquidAI/LFM2.5-350M-ONNX';
      const revision = 'd11593fd9eb408e322667926656598896c2d5ff9';
      const h = await connectRawDownload({ modelId, revision, remoteRefs: new Map() });
      try {
