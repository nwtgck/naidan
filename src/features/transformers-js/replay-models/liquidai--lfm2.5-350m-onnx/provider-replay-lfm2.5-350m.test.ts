...
describe('LFM2.5 350M Provider / basic', () => {
  it('preserves the recorded first user input without supplying any output tokens', async () => {
    ...
    const harness = await createProviderReplayTestRuntime({
      modelId: 'LiquidAI/LFM2.5-350M-ONNX', expectedRevision: source.identity.resolvedRevision, cacheRevision: source.identity.resolvedRevision, metadataCache: "all-fixture", imagePlatform: undefined,
  ...
  it('delivers the recorded first-turn stream exactly before Provider settlement', async () => {
    ...
    const harness = await createProviderReplayTestRuntime({
      ...
      modelId: 'LiquidAI/LFM2.5-350M-ONNX', expectedRevision: evidence.identity.resolvedRevision, cacheRevision: evidence.identity.resolvedRevision, metadataCache: "all-fixture",
  ...
  it('basic: delivers the recorded first-turn callbacks before settlement', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["first-turn"], artifactPaths: ["onnx/model_q4f16.onnx","onnx/model_q4f16.onnx_data"], imagePlatform: undefined });
  }, 30_000);
});

describe('LFM2.5 350M Provider / system', () => {
  it('system: preserves instructions and delivers the recorded callbacks', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["system-user"], artifactPaths: ["onnx/model_q4f16.onnx","onnx/model_q4f16.onnx_data"], imagePlatform: undefined });
  }, 30_000);
});
...
describe('LFM2.5 350M Provider / history', () => {
  ...
  it('preserves the captured follow-up prefix with explicitly supplied assistant history', async () => {
    ...
    const harness = await createProviderReplayTestRuntime({
      imagePlatform: undefined,
      modelId: 'LiquidAI/LFM2.5-350M-ONNX', expectedRevision: continuity.identity.resolvedRevision, cacheRevision: continuity.identity.resolvedRevision, metadataCache: "all-fixture",
      artifacts: [
...
`, '"Thank ', 'you ', 'for ', 'your ', 'feedback."',
  ...
  it('history: preserves supplied history and delivers the recorded callbacks', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["supplied-history"], artifactPaths: ["onnx/model_q4f16.onnx","onnx/model_q4f16.onnx_data"], imagePlatform: undefined });
  }, 30_000);
});
...
`;
  ...
  it('independent: keeps a new conversation independent after settled requests in the same runtime', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["first-turn","continuity","independent-next-input"], artifactPaths: ["onnx/model_q4f16.onnx","onnx/model_q4f16.onnx_data"], imagePlatform: undefined });
  }, 30_000);
});

describe('LFM2.5 350M Provider / reasoning', () => {
  it('reasoning: preserves the recorded none-effort request and callbacks', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["reasoning-none"], artifactPaths: ["onnx/model_q4f16.onnx","onnx/model_q4f16.onnx_data"], imagePlatform: undefined });
  }, 30_000);
  it('reasoning: preserves the recorded low-effort request and callbacks', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["reasoning-low"], artifactPaths: ["onnx/model_q4f16.onnx","onnx/model_q4f16.onnx_data"], imagePlatform: undefined });
  }, 30_000);
  it('reasoning: preserves the recorded medium-effort request and callbacks', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["reasoning-medium"], artifactPaths: ["onnx/model_q4f16.onnx","onnx/model_q4f16.onnx_data"], imagePlatform: undefined });
  }, 30_000);
  it('reasoning: preserves the recorded high-effort request and callbacks', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["reasoning-high"], artifactPaths: ["onnx/model_q4f16.onnx","onnx/model_q4f16.onnx_data"], imagePlatform: undefined });
  }, 30_000);
});
...
describe('LFM2.5 350M Provider / tools', () => {
  it.each(toolInputEvidence.cases)('$caseId retains public tool definitions and result content, but the native template omits call details', async scenario => {
    ...
    let inference: Parameters<ProviderReplayGenerate>[0] | undefined;
  ...
  it('tools: preserves the recorded minimal no-call response without claiming model-wide non-support', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["natural-tool-minimal"], artifactPaths: ["onnx/model_q4f16.onnx","onnx/model_q4f16.onnx_data"], imagePlatform: undefined });
  }, 30_000);
  it('tools: preserves the recorded representative no-call response without claiming model-wide non-support', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["natural-tool-representative"], artifactPaths: ["onnx/model_q4f16.onnx","onnx/model_q4f16.onnx_data"], imagePlatform: undefined });
  }, 30_000);
  it('tools: preserves structured caller history and the recorded response', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["structured-tool-history"], artifactPaths: ["onnx/model_q4f16.onnx","onnx/model_q4f16.onnx_data"], imagePlatform: undefined });
  }, 30_000);
});
...
describe('LFM2.5 350M Provider / images', () => {
  it('images: preserves the recorded text-only native handling of an image-bearing request', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["image"], artifactPaths: ["onnx/model_q4f16.onnx","onnx/model_q4f16.onnx_data"], imagePlatform: undefined });
  }, 30_000);
});
...
describe('LFM2.5 350M Provider / sequences', () => {
  it('uses only the settled first callback text for a second request in the same loaded runtime', async () => {
    ...
    const harness = await createProviderReplayTestRuntime({
      modelId: 'LiquidAI/LFM2.5-350M-ONNX', expectedRevision: evidence.identity.resolvedRevision, cacheRevision: evidence.identity.resolvedRevision, metadataCache: "all-fixture", imagePlatform: undefined,
      artifacts: ['onnx/model_q4f16.onnx', 'onnx/model_q4f16.onnx_data'].map(path => ({ path, bytes: createSyntheticModelBody({ modelId: inputEvidence.modelId, revision: inputEvidence.revision, path }) })),
      generate: async context => {
        const next = invocations.shift();
  ...
  it('sequences: builds continuation from actually delivered first-request settlement', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["first-turn","continuity"], artifactPaths: ["onnx/model_q4f16.onnx","onnx/model_q4f16.onnx_data"], imagePlatform: undefined });
  }, 30_000);
  it('preserves thirteen causal requests, native streams and settlements in one Load', async () => {
    ...
    await verifyCapturedFullReplay({ unavailableOutputs: [], completeResult: undefined, expectedLoadReceipt: undefined, evidence: fullEvidenceJson, imagePlatform: undefined, artifactPaths: ["onnx/model_q4f16.onnx","onnx/model_q4f16.onnx_data"] });
  }, 30_000);
});
