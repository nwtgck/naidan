...
async function createLfm230InputReplay() {
  ...
  const harness = await createProviderReplayTestRuntime({
    modelId: MODEL_ID, expectedRevision: REVISION, cacheRevision: REVISION, metadataCache: "all-fixture", imagePlatform: undefined,
...
` },
  ...
  it('basic: delivers the recorded first-turn callbacks before settlement', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["first-turn"], artifactPaths: ["onnx/model_q4.onnx","onnx/model_q4.onnx_data"], imagePlatform: undefined });
  }, 30_000);
});

describe('LFM2.5 230M Provider / system', () => {
  it('system: preserves instructions and delivers the recorded callbacks', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["system-user"], artifactPaths: ["onnx/model_q4.onnx","onnx/model_q4.onnx_data"], imagePlatform: undefined });
  }, 30_000);
});
...
`;
    ...
    const harness = await createProviderReplayTestRuntime({
      ...
      modelId: MODEL_ID, expectedRevision: REVISION, cacheRevision: REVISION, metadataCache: "all-fixture",
  ...
  it('history: preserves supplied history and delivers the recorded callbacks', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["supplied-history"], artifactPaths: ["onnx/model_q4.onnx","onnx/model_q4.onnx_data"], imagePlatform: undefined });
  }, 30_000);
});
...
describe('LFM2.5 230M Provider / independent', () => {
  it('independent: keeps a new conversation independent after settled requests in the same runtime', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["first-turn","continuity","independent-next-input"], artifactPaths: ["onnx/model_q4.onnx","onnx/model_q4.onnx_data"], imagePlatform: undefined });
  }, 30_000);
});

describe('LFM2.5 230M Provider / reasoning', () => {
  it('reasoning: preserves the recorded none-effort request and callbacks', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["reasoning-none"], artifactPaths: ["onnx/model_q4.onnx","onnx/model_q4.onnx_data"], imagePlatform: undefined });
  }, 30_000);
  it('reasoning: preserves the recorded low-effort request and callbacks', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["reasoning-low"], artifactPaths: ["onnx/model_q4.onnx","onnx/model_q4.onnx_data"], imagePlatform: undefined });
  }, 30_000);
  it('reasoning: preserves the recorded medium-effort request and callbacks', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["reasoning-medium"], artifactPaths: ["onnx/model_q4.onnx","onnx/model_q4.onnx_data"], imagePlatform: undefined });
  }, 30_000);
  it('reasoning: preserves the recorded high-effort request and callbacks', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["reasoning-high"], artifactPaths: ["onnx/model_q4.onnx","onnx/model_q4.onnx_data"], imagePlatform: undefined });
  }, 30_000);
});
...
`;
  ...
  it('tools: executes the recorded minimal Tokyo call once and continues with its result', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["natural-tool-minimal"], artifactPaths: ["onnx/model_q4.onnx","onnx/model_q4.onnx_data"], imagePlatform: undefined });
  }, 30_000);
  it('tools: executes the recorded representative Tokyo call once and continues with its result', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["natural-tool-representative"], artifactPaths: ["onnx/model_q4.onnx","onnx/model_q4.onnx_data"], imagePlatform: undefined });
  }, 30_000);
  it('tools: preserves structured caller history and the recorded response', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["structured-tool-history"], artifactPaths: ["onnx/model_q4.onnx","onnx/model_q4.onnx_data"], imagePlatform: undefined });
  }, 30_000);
});
...
describe('LFM2.5 230M Provider / images', () => {
  it('images: preserves the recorded text-only native handling of an image-bearing request', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["image"], artifactPaths: ["onnx/model_q4.onnx","onnx/model_q4.onnx_data"], imagePlatform: undefined });
  }, 30_000);
});
...
describe('LFM2.5 230M Provider / sequences', () => {
  it('sequences: builds continuation from actually delivered first-request settlement', async () => {
    await verifyProviderRequests({ catalog: providerReplayCatalog, caseIds: ["first-turn","continuity"], artifactPaths: ["onnx/model_q4.onnx","onnx/model_q4.onnx_data"], imagePlatform: undefined });
  }, 30_000);
  it('preserves thirteen causal requests, native streams and settlements in one Load', async () => {
    ...
    await verifyCapturedFullReplay({ unavailableOutputs: [], completeResult: undefined, expectedLoadReceipt: undefined, evidence: fullEvidenceJson, imagePlatform: undefined, artifactPaths: ["onnx/model_q4.onnx","onnx/model_q4.onnx_data"] });
  }, 30_000);
});
