...
it('LFM2.5 2.6B freshly investigates metadata with absent Content-Length and 206 probes without model downloads', async () => {
...
it('LFM2.5 2.6B excludes a candidate with missing onnx/model_q4f16.onnx_data_1 before ORT without repairing it', async () => {
  const modelId = 'LiquidAI/LFM2.5-2.6B-ONNX';
  const revision = '66826372fd4fa166f53be0371c9315745c07cace';
...
it('LFM2.5 2.6B rejects missing required tokenizer.json during offline Load without repairing it', async () => {
  const modelId = 'LiquidAI/LFM2.5-2.6B-ONNX';
  const revision = '66826372fd4fa166f53be0371c9315745c07cace';
...
it('LFM2.5 2.6B exact Download preserves its selected artifacts through fresh offline Load', async () => {
...
it('LFM2.5 2.6B service Download reuses its committed candidate and cold-loads offline', async () => {
  const modelId = 'LiquidAI/LFM2.5-2.6B-ONNX';
  const revision = '66826372fd4fa166f53be0371c9315745c07cace';
  const expectedPaths = [
    'onnx/model_q4f16.onnx',
