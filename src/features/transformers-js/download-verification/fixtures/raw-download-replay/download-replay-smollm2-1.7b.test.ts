...
it('SmolLM2 1.7B preserves prepared metadata when its supplemental special-token map times out', async () => {
...
it('SmolLM2 1.7B freshly investigates metadata with absent Content-Length and 206 probes without model downloads', async () => {
  const modelId = 'HuggingFaceTB/SmolLM2-1.7B-Instruct';
  const revision = '31b70e2e869a7173562077fd711b654946d38674';
  const h = await connectRawDownload({ modelId, revision, remoteRefs: new Map() });
...
it('SmolLM2 1.7B rejects corrupt optional configuration before reading any model body or creating an ORT session', async () => {
  const modelId = 'HuggingFaceTB/SmolLM2-1.7B-Instruct';
  const revision = '31b70e2e869a7173562077fd711b654946d38674';
  const h = await connectRawDownload({ modelId, revision, remoteRefs: new Map() });
  try {
...
it('SmolLM2 1.7B loads valid present optional generation configuration offline without another Download or cache mutation', async () => {
  const modelId = 'HuggingFaceTB/SmolLM2-1.7B-Instruct';
  const revision = '31b70e2e869a7173562077fd711b654946d38674';
  const h = await connectRawDownload({ modelId, revision, remoteRefs: new Map() });
  try {
...
it('SmolLM2 1.7B rejects an OPFS read failure for present optional generation metadata instead of silently accepting the model', async () => {
  const modelId = 'HuggingFaceTB/SmolLM2-1.7B-Instruct';
  const revision = '31b70e2e869a7173562077fd711b654946d38674';
  const h = await connectRawDownload({ modelId, revision, remoteRefs: new Map() });
...
it('SmolLM2 1.7B stops candidate orchestration when present optional generation metadata is corrupt instead of downloading another dtype', async () => {
  const modelId = 'HuggingFaceTB/SmolLM2-1.7B-Instruct';
  const revision = '31b70e2e869a7173562077fd711b654946d38674';
...
it('SmolLM2 1.7B immutable offline Load does not read shadow metadata from the local user namespace', async () => {
  const modelId = 'HuggingFaceTB/SmolLM2-1.7B-Instruct';
  const revision = '31b70e2e869a7173562077fd711b654946d38674';
  const h = await connectRawDownload({ modelId, revision, remoteRefs: new Map() });
  try {
...
it('SmolLM2 1.7B excludes a candidate with missing onnx/model_q4f16.onnx before ORT without repairing it', async () => {
  const modelId = 'HuggingFaceTB/SmolLM2-1.7B-Instruct';
  const revision = '31b70e2e869a7173562077fd711b654946d38674';
...
it('SmolLM2 1.7B rejects missing required tokenizer.json during offline Load without repairing it', async () => {
  const modelId = 'HuggingFaceTB/SmolLM2-1.7B-Instruct';
  const revision = '31b70e2e869a7173562077fd711b654946d38674';
...
it('SmolLM2 1.7B exact Download preserves its selected artifacts through fresh offline Load', async () => {
...
it('SmolLM2 1.7B service Download reuses its committed candidate and cold-loads offline', async () => {
  const modelId = 'HuggingFaceTB/SmolLM2-1.7B-Instruct';
  const revision = '31b70e2e869a7173562077fd711b654946d38674';
  const expectedPaths = [
    'onnx/model_q4f16.onnx',
