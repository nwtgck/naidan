...
it('Qwen3.5 4B freshly investigates metadata with absent Content-Length and 206 probes without model downloads', async () => {
...
it('Qwen3.5 4B excludes a candidate with missing onnx/decoder_model_merged_q4f16.onnx_data_1 before ORT without repairing it', async () => {
  const modelId = 'onnx-community/Qwen3.5-4B-ONNX';
  const revision = '74d8caba2117fd5f41d655e9cc27eda1338662b3';
  const missingPath = 'onnx/decoder_model_merged_q4f16.onnx_data_1';
  const h = await connectRawDownload({ modelId, revision, remoteRefs: new Map() });
  try {
...
it('Qwen3.5 4B rejects missing required preprocessor_config.json during offline Load without repairing it', async () => {
  const modelId = 'onnx-community/Qwen3.5-4B-ONNX';
  const revision = '74d8caba2117fd5f41d655e9cc27eda1338662b3';
...
it('Qwen3.5 4B exact Download preserves its selected artifacts through fresh offline Load', async () => {
  // Fixed original metadata and complete repository inventory, not Production output.
  const modelId = 'onnx-community/Qwen3.5-4B-ONNX';
  const revision = '74d8caba2117fd5f41d655e9cc27eda1338662b3';
  const expectedArtifacts = [
    { path: 'onnx/decoder_model_merged_q4f16.onnx', size: 1421099 },
    { path: 'onnx/decoder_model_merged_q4f16.onnx_data', size: 2067208192 },
    { path: 'onnx/decoder_model_merged_q4f16.onnx_data_1', size: 367513600 },
    { path: 'onnx/embed_tokens_q4f16.onnx', size: 1064 },
    { path: 'onnx/embed_tokens_q4f16.onnx_data', size: 367513600 },
  ];
  // CausalLM consumes decoder/embed. Repository-present vision probes are not downloads.
  const h = await connectRawDownload({ modelId, revision, remoteRefs: new Map() });
  try {
    expect(digest({ bytes: h.archive.files.get('config.json')! })).toBe('c6f9834460177e3821e035900320fa24bd11ad1c9f14bfe2e78e4398e38c4937');
    ...
    const base = `models/huggingface.co/${modelId}/resolve/${revision}/`;
    // Qwen3VLProcessor inherits uses_processor_config=false; its image component
    // reads the original preprocessor_config, not the separate processor_config.
...
it('Qwen3.5 4B service Download reuses its committed candidate and cold-loads offline', async () => {
  const modelId = 'onnx-community/Qwen3.5-4B-ONNX';
  const revision = '74d8caba2117fd5f41d655e9cc27eda1338662b3';
  const expectedPaths = [
    'onnx/decoder_model_merged_q4f16.onnx',
