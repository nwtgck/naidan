...
Each model directory owns its `model.evidence.json` and original metadata.
Large `tokenizer.json.gz` assets are losslessly compressed single JSON files,
...
```sh
npm run test:only-failed -- src/features/transformers-js/model-support-investigation/logic/fixtures/raw-metadata-replay --maxWorkers=1
npm run test:only-failed -- src/features/transformers-js/download-verification/fixtures/raw-download-replay --maxWorkers=1
```

Each `metadata-replay-<model>.test.ts` and `download-replay-<model>.test.ts`
