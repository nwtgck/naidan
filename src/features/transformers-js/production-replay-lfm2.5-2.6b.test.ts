...
import inputJson from './production-replay-lfm2.5-2.6b.input.evidence.json';
import toolInputJson from './production-replay-lfm2.5-2.6b.tool-input.evidence.json';
...
import evidenceJson from './production-replay-lfm2.5-2.6b.evidence.json';
import rawHistoryJson from './production-replay-lfm2.5-2.6b.raw-history.evidence.json';
...
const rawHistory = parseProductionReplayTextEvidence({ value: rawHistoryJson });
...
describe('LFM2.5 2.6B captured Production Provider replay', () => {
  it('preserves the recorded first user input without supplying any output tokens', async () => {
    ...
    const harness = await createProductionReplayTestRuntime({
      modelId: 'LiquidAI/LFM2.5-2.6B-ONNX', expectedRevision: source.identity.resolvedRevision, imagePlatform: undefined,
...
describe('LFM2.5 2.6B real Production system and history input', () => {
...
describe('LFM2.5 2.6B captured Production Provider replay', () => {
  it('delivers the recorded first-turn stream exactly before Provider settlement', async () => {
    ...
    const harness = await createProductionReplayTestRuntime({
      ...
      modelId: 'LiquidAI/LFM2.5-2.6B-ONNX', expectedRevision: evidence.identity.resolvedRevision,
...
describe('LFM2.5 2.6B public strict-schema and mapping tool inputs', () => {
