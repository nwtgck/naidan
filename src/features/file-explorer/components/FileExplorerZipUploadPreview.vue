...
function getActionBadgeClasses({ action }: { action: FileExplorerZipUploadPreviewAction }): TailwindClass {
  switch (action) {
  case 'existing':
    return twClassString('text-gray-500', 'bg-gray-100', 'dark:bg-gray-800');
  case 'add':
  case 'merge':
    return twClassString('text-blue-600', 'dark:text-blue-300', 'bg-blue-50', 'dark:bg-blue-950/40');
  case 'replace':
    return twClassString('text-amber-700', 'dark:text-amber-300', 'bg-amber-50', 'dark:bg-amber-950/40');
  case 'blocked':
    return twClassString('text-red-700', 'dark:text-red-300', 'bg-red-50', 'dark:bg-red-950/40');
  default: {
    const _exhaustiveCheck: never = action;
    throw new Error(`Unhandled ZIP preview action: ${String(_exhaustiveCheck)}`);
  }
...
<template>
  <section tw-class="min-w-0 flex flex-col bg-white dark:bg-gray-900" data-testid="zip-upload-preview">
    <header tw-class="flex items-center justify-between gap-3 min-h-[52px] px-4 py-2.5 border-b border-gray-200 dark:border-gray-700">
      <h3 tw-class="text-xs font-bold text-gray-800 dark:text-gray-200">
        {{ lazyStrings.fileExplorer__placement_preview() }}
      </h3>
      <span tw-class="px-2 py-1 text-[9px] font-bold text-blue-600 dark:text-blue-300 bg-blue-50 dark:bg-blue-950/40 rounded-full">
        {{ lazyStrings.fileExplorer__not_changed_yet() }}
      </span>
    </header>

    <nav tw-class="flex items-center gap-1.5 min-h-10 px-3 py-1.5 bg-gray-50 dark:bg-gray-950/30 border-b border-gray-200 dark:border-gray-700 text-[10px] font-mono text-gray-500">
      <button
        type="button"
        tw-class="inline-flex items-center gap-1 hover:text-blue-500"
        data-testid="zip-preview-root"
        @click="ctx.upload.navigatePreview({ relativePath: '' })"
      >
        <FolderIcon tw-class="w-3.5 h-3.5 text-yellow-500" />
        {{ state.targetDirectoryPath }}
      </button>
      <template v-for="segment in state.previewPathSegments" :key="segment.relativePath">
        <ChevronRightIcon tw-class="w-3 h-3 text-gray-400" />
        <button
          type="button"
          tw-class="hover:text-blue-500 truncate"
    ...
    <div tw-class="grid grid-cols-[minmax(150px,1fr)_64px_104px] items-center gap-2 min-h-[29px] px-3 text-[9px] font-bold uppercase tracking-wide text-gray-400 bg-gray-50 dark:bg-gray-950/30 border-b border-gray-200 dark:border-gray-700">
      <span>{{ lazyStrings.fileExplorer__name() }}</span>
      <span tw-class="text-right">{{ lazyStrings.fileExplorer__size() }}</span>
      <span tw-class="text-right">{{ lazyStrings.fileExplorer__status() }}</span>
    </div>

    <div ref="scrollContainerRef" tw-class="min-h-[230px] flex-1 overflow-y-auto p-1.5">
      ...
      <FileExplorerListEntryRow
        ...
        :tw-class="['cursor-default', previewEntry.canNavigate ? 'cursor-pointer' : '']"
        :data-testid="`zip-preview-entry-${previewEntry.name}`"
        @dblclick="openEntry({ entry: previewEntry })"
      >
        <template #trailing>
          <span tw-class="text-[10px] font-mono w-16 text-right text-gray-400 dark:text-gray-500">
            {{ previewEntry.kind === 'file' ? formatSize({ bytes: previewEntry.size }) : '' }}
          </span>
          <span
            :tw-class="['justify-self-end px-2 py-1 text-[8px] font-bold rounded-full whitespace-nowrap', twClasses(getActionBadgeClasses({ action: previewEntry.action }))]"
          >
    ...
    <footer tw-class="flex items-center flex-wrap gap-1.5 min-h-11 px-3 py-2 bg-gray-50 dark:bg-gray-950/30 border-t border-gray-200 dark:border-gray-700">
      <span v-if="state.previewSummary.addedCount > 0" tw-class="px-2 py-1 text-[9px] font-bold text-blue-600 bg-blue-50 dark:bg-blue-950/40 rounded-full">
        {{ lazyStrings.fileExplorer__addition_count({ count: state.previewSummary.addedCount }) }}
      </span>
      <span v-if="state.previewSummary.mergedCount > 0" tw-class="px-2 py-1 text-[9px] font-bold text-blue-600 bg-blue-50 dark:bg-blue-950/40 rounded-full">
        {{ lazyStrings.fileExplorer__merge_count({ count: state.previewSummary.mergedCount }) }}
      </span>
      <span v-if="state.previewSummary.replacedCount > 0" tw-class="px-2 py-1 text-[9px] font-bold text-amber-700 bg-amber-50 dark:bg-amber-950/40 rounded-full">
        {{ lazyStrings.fileExplorer__overwrite_count({ count: state.previewSummary.replacedCount }) }}
      </span>
      <span v-if="state.previewSummary.blockedCount > 0" tw-class="px-2 py-1 text-[9px] font-bold text-red-700 bg-red-50 dark:bg-red-950/40 rounded-full">
