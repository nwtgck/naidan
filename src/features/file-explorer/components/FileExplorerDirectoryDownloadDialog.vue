...
<template>
  <Teleport to="body">
    <div
      ...
      tw-class="fixed inset-0 z-[600] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"
      data-testid="directory-download-overlay"
      @click.self="controller.close()"
      @keydown.esc.stop.prevent="controller.close()"
    >
      <section
        tw-class="w-full max-w-[560px] overflow-visible bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-2xl"
        role="dialog"
        aria-modal="true"
        :aria-label="lazyStrings.fileExplorer__download_directory()"
        data-testid="directory-download-dialog"
      >
        <header tw-class="flex items-center justify-between min-h-[50px] px-4 py-2.5 border-b border-gray-200 dark:border-gray-700">
          <h2 tw-class="text-sm font-semibold text-gray-700 dark:text-gray-300">
            {{ lazyStrings.fileExplorer__download_directory() }}
          </h2>
          <button
            type="button"
            tw-class="grid place-items-center w-7 h-7 text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg"
        ...
        <div tw-class="grid gap-5 px-5 py-[18px]">
          <label tw-class="grid gap-2">
            <span tw-class="text-xs font-bold text-gray-700 dark:text-gray-300">
              {{ lazyStrings.fileExplorer__archive_name() }}
            </span>
            <span tw-class="flex items-stretch">
              <input
                ref="archiveNameInput"
                :value="state.archiveName"
                type="text"
                tw-class="min-w-0 flex-1 h-[38px] px-3 text-xs text-gray-700 dark:text-gray-300 bg-transparent border border-gray-200 dark:border-gray-700 rounded-l-lg outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15 disabled:opacity-50"
              ...
              />
              <span tw-class="flex items-center px-3 text-[11px] font-mono text-gray-400 bg-gray-50 dark:bg-gray-900/80 border border-l-0 border-gray-200 dark:border-gray-700 rounded-r-lg">
                .zip
              </span>
            </span>
          </label>
          ...
          <div tw-class="grid gap-2">
            <div tw-class="flex items-baseline justify-between gap-3">
              <span tw-class="text-xs font-bold text-gray-700 dark:text-gray-300">
                {{ lazyStrings.fileExplorer__exclude_items() }}
              </span>
              <span tw-class="text-[10px] text-gray-400">
            ...
            >
              <div tw-class="min-h-[72px] p-2 bg-transparent border border-gray-200 dark:border-gray-700 rounded-lg focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/15">
                <div v-if="state.exclusions.length > 0" tw-class="flex flex-wrap gap-1.5 mb-1.5">
                  <span
                    v-for="exclusion in state.exclusions"
                    :key="exclusion.relativePath"
                    tw-class="inline-flex items-center gap-1.5 max-w-full min-h-[25px] px-2 py-1 text-[11px] font-mono text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40 rounded-md"
                    data-testid="directory-download-exclusion-chip"
                  >
                    <FolderIcon v-if="exclusion.kind === 'directory'" tw-class="w-3 h-3 shrink-0" />
                    <FileIcon v-else tw-class="w-3 h-3 shrink-0" />
                    <span tw-class="truncate">{{ exclusion.relativePath }}{{ exclusion.kind === 'directory' ? '/' : '' }}</span>
                    <button
                      type="button"
                      tw-class="grid place-items-center w-[18px] h-[18px] shrink-0 text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-800 rounded"
                      :disabled="state.creationStatus === 'creating'"
                ...
                <div tw-class="flex items-center gap-2 min-h-[27px]">
                  <PlusIcon tw-class="w-4 h-4 shrink-0 text-gray-400" />
                  <input
                    :value="state.query"
                    type="text"
                    tw-class="w-full min-w-0 py-1 text-[11px] font-mono text-gray-700 dark:text-gray-300 bg-transparent border-0 outline-none placeholder:text-gray-400"
                  ...
                  <button
                    type="button"
                    tw-class="shrink-0 min-h-[26px] px-2.5 text-[10px] font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40 rounded-md hover:bg-blue-100 dark:hover:bg-blue-950/70 disabled:opacity-40 disabled:cursor-not-allowed"
              ...
              <div
                ...
                tw-class="max-h-40 overflow-y-auto mt-1.5 py-1 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg"
                role="listbox"
                data-testid="directory-download-suggestions"
              >
                <div
                  v-if="state.suggestionStatus === 'loading'"
                  tw-class="flex items-center gap-2 px-3 py-2 text-[11px] text-gray-400"
                >
                  <Loader2Icon tw-class="w-3.5 h-3.5 animate-spin" />
                </div>
                <div
                  v-else-if="state.suggestionStatus === 'error'"
                  tw-class="px-3 py-2 text-[11px] text-gray-400"
                >
                ...
                <template v-else-if="state.suggestions.length > 0">
                  <button
                    ...
                    type="button"
                    :tw-class="['grid grid-cols-[18px_minmax(0,1fr)_auto] items-center gap-2 w-full min-h-8 px-3 py-1.5 text-left text-[11px] hover:bg-gray-100 dark:hover:bg-gray-800', index === state.selectedSuggestionIndex ? 'bg-gray-100 dark:bg-gray-800' : '']"
                    role="option"
                  ...
                  >
                    <FolderIcon v-if="suggestion.kind === 'directory'" tw-class="w-4 h-4 text-yellow-500" />
                    <FileIcon v-else tw-class="w-4 h-4 text-gray-400" />
                    <span tw-class="truncate font-mono text-gray-700 dark:text-gray-300">
                  ...
                  <div
                    ...
                    tw-class="px-3 py-2 text-[10px] text-gray-400 border-t border-gray-100 dark:border-gray-800"
                  >
                    {{ lazyStrings.fileExplorer__type_to_narrow_results() }}
                  </div>
                </template>
                <div v-else tw-class="px-3 py-2 text-[11px] text-gray-400">
                  {{ lazyStrings.fileExplorer__no_matching_items() }}
                </div>
              </div>
            </div>

            <p tw-class="m-0 text-[10px] leading-4 text-gray-400">
              {{ lazyStrings.fileExplorer__exclude_items_help() }}
            </p>
          </div>
        </div>

        <footer tw-class="flex items-center justify-end gap-2 min-h-[58px] px-4 py-2.5 bg-gray-50/80 dark:bg-gray-950/20 border-t border-gray-200 dark:border-gray-700 rounded-b-2xl">
          <button
            type="button"
            tw-class="min-h-[34px] px-3.5 py-1.5 text-[11px] font-bold text-gray-700 dark:text-gray-300 bg-transparent border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800"
          ...
          <button
            type="button"
            tw-class="inline-flex items-center gap-2 min-h-[34px] px-3.5 py-1.5 text-[11px] font-bold text-white bg-blue-500 border border-blue-500 rounded-lg hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          >
            <Loader2Icon v-if="state.creationStatus === 'creating'" tw-class="w-3.5 h-3.5 animate-spin" />
            <DownloadIcon v-else tw-class="w-3.5 h-3.5" />
