...
<template>
  <Teleport to="body">
    <div
      ...
      tw-class="fixed inset-0 z-[600] flex items-center justify-center p-4 bg-black/55"
      data-testid="zip-upload-overlay"
      @click.self="!isBusy && ctx.upload.close()"
      @keydown.esc.stop.prevent="!isBusy && ctx.upload.close()"
    >
      <section
        tw-class="w-full max-w-[980px] max-h-[calc(100vh-32px)] flex flex-col overflow-hidden bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-2xl"
        role="dialog"
        aria-modal="true"
        :aria-label="lazyStrings.fileExplorer__zip_file_upload()"
        data-testid="zip-upload-dialog"
      >
        <header tw-class="flex items-center justify-between min-h-[52px] px-4 py-2.5 border-b border-gray-200 dark:border-gray-700">
          <div tw-class="min-w-0 flex items-center gap-2">
            <h2 tw-class="text-sm font-semibold text-gray-700 dark:text-gray-300">
              {{ lazyStrings.fileExplorer__zip_file_upload() }}
            </h2>
            <span v-if="state.totalZipCount > 1" tw-class="text-[10px] text-gray-400">
              {{ state.currentZipIndex + 1 }} / {{ state.totalZipCount }}
            </span>
          </div>
          <button
            type="button"
            tw-class="grid place-items-center w-7 h-7 text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg disabled:opacity-40"
        ...
        <div tw-class="min-h-0 grid grid-cols-[minmax(320px,410px)_minmax(380px,1fr)] max-[760px]:block flex-1 overflow-hidden max-[760px]:overflow-y-auto">
          <section tw-class="grid content-start gap-5 p-[18px] overflow-y-auto border-r border-gray-200 dark:border-gray-700 max-[760px]:overflow-visible max-[760px]:border-r-0 max-[760px]:border-b">
            <div tw-class="flex items-center gap-3 min-w-0 p-3 bg-gray-50 dark:bg-gray-950/30 border border-gray-200 dark:border-gray-700 rounded-lg">
              <span tw-class="grid place-items-center w-9 h-9 shrink-0 text-amber-600 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/30 rounded-lg">
                <ArchiveIcon tw-class="w-5 h-5" />
              </span>
              <span tw-class="min-w-0">
                <span tw-class="block truncate text-xs font-semibold text-gray-800 dark:text-gray-200">
                  {{ state.currentFileName }}
                </span>
                <span tw-class="block mt-0.5 text-[10px] text-gray-400">
                  {{ lazyStrings.fileExplorer__zip_archive() }}
                </span>
              </span>
            </div>

            <div v-if="state.phase === 'analyzing'" tw-class="flex items-center gap-2 text-xs text-gray-400">
              <Loader2Icon tw-class="w-4 h-4 animate-spin" />
              {{ lazyStrings.fileExplorer__analyzing_zip() }}
            </div>

            <fieldset v-else tw-class="grid gap-2" :disabled="isBusy">
              <legend tw-class="mb-2 text-xs font-bold text-gray-700 dark:text-gray-300">
                {{ lazyStrings.fileExplorer__placement_method() }}
              </legend>

              <label
                :tw-class="['grid grid-cols-[18px_minmax(0,1fr)] gap-2.5 p-3 border rounded-lg cursor-pointer', state.placement.kind === 'keep_archive'
              ...
              >
                <input
                  type="radio"
                  name="zip-placement"
                  tw-class="mt-0.5 accent-blue-500"
                  :checked="state.placement.kind === 'keep_archive'"
                  data-testid="zip-placement-keep"
                  @change="selectKeepArchive()"
                />
                <span>
                  <span tw-class="block text-[11px] font-bold text-gray-800 dark:text-gray-200">
                    {{ lazyStrings.fileExplorer__place_zip_file_as_is() }}
                  </span>
                  <span tw-class="block mt-1 text-[10px] leading-4 text-gray-500">
              ...
              <fieldset
                ...
                tw-class="grid gap-2 ml-7 p-3 bg-gray-50 dark:bg-gray-950/30 border border-gray-200 dark:border-gray-700 rounded-lg"
              >
                <legend tw-class="px-1 text-[11px] font-bold text-gray-700 dark:text-gray-300">
                  {{ lazyStrings.fileExplorer__root_directory_handling({ name: state.singleRootDirectoryName }) }}
                </legend>

                <label tw-class="grid grid-cols-[18px_minmax(0,1fr)] gap-2 p-2.5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg cursor-pointer">
                  <input
                    type="radio"
            ...
            <div v-if="state.errorMessage" tw-class="px-3 py-2 text-[10px] leading-4 text-red-700 dark:text-red-300 bg-red-50 dark:bg-red-950/30 rounded-lg" data-testid="zip-upload-error">
              {{ state.errorMessage }}
            </div>
          </section>
        ...
        <footer tw-class="flex items-center justify-end gap-2 min-h-[58px] px-4 py-2.5 bg-gray-50/80 dark:bg-gray-950/20 border-t border-gray-200 dark:border-gray-700">
          <button
            type="button"
            tw-class="min-h-[34px] px-3.5 py-1.5 text-[11px] font-bold text-gray-700 dark:text-gray-300 bg-transparent border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 disabled:opacity-50"
          ...
          >
            <Loader2Icon v-if="state.phase === 'uploading'" tw-class="w-3.5 h-3.5 animate-spin" />
            <UploadIcon v-else tw-class="w-3.5 h-3.5" />
