...
<template>
  <div tw-class="flex items-center gap-1 min-w-0 flex-1">
    <!-- Back button -->
    <button
      :disabled="ctx.pathSegments.length <= 1"
      tw-class="p-1.5 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 disabled:opacity-30 transition-colors shrink-0"
      :title="lazyStrings.fileExplorer__go_back()"
      data-testid="breadcrumb-back"
      @click="ctx.navigateUp()"
    >
      <ChevronLeftIcon tw-class="w-3.5 h-3.5 text-gray-500" />
    </button>
    ...
    <input
      ...
      tw-class="flex-1 px-2 py-1 text-xs bg-white dark:bg-gray-800 border border-blue-500 rounded-lg outline-none focus:ring-2 focus:ring-blue-500/20 text-gray-800 dark:text-gray-200"
    ...
    <div
      v-else
      tw-class="flex items-center gap-0.5 overflow-hidden min-w-0 cursor-text flex-1"
    ...
    >
      <template v-for="(seg, i) in ctx.pathSegments" :key="i">
        <button
          ...
          tw-class="px-1.5 py-0.5 text-[11px] text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 rounded transition-colors shrink-0 max-w-[120px] truncate"
          :title="seg.name"
        ...
        <ChevronRightIcon
          v-if="i < ctx.pathSegments.length - 1"
          tw-class="w-3 h-3 text-gray-300 dark:text-gray-600 shrink-0"
        />
        <span
          v-if="i === ctx.pathSegments.length - 1"
          tw-class="px-1.5 py-0.5 text-[11px] font-bold text-gray-800 dark:text-gray-200 truncate"
