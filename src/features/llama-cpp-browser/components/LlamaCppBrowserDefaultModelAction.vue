...
<template>
  <button type="button" data-testid="llama-default-model-action" :disabled="disabled || selected" :aria-pressed="selected" :tw-class="['inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-500 disabled:cursor-not-allowed', selected ? 'text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/20' : 'border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-purple-600 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-900/20 disabled:opacity-50']" @click="emit('select', model)">
