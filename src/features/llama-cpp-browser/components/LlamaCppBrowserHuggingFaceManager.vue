...
import { AlertCircleIcon, ChevronDownIcon, DownloadIcon, HardDriveIcon, Loader2Icon, PauseIcon, PlayIcon, SearchIcon, Trash2Icon } from 'lucide-vue-next';
import { quantizationChoices, preferredProjector } from '@/features/llama-cpp-browser/hugging-face/presentation';
...
import { DownloadConflictError, type DownloadConflict, type DownloadJournal, type DownloadProgress, type DownloadSelection } from '@/features/llama-cpp-browser/hugging-face/types';
...
const choices = computed(() => quantizationChoices({ models: catalog.value?.models ?? [] }));
const selectedChoice = computed(() => choices.value.find(choice => (choice.quantization ?? 'unknown') === quantization.value));
...
async function inspect(): Promise<void> {
  ...
  try {
    ...
    const nextChoices = quantizationChoices({ models: next.models });
    if (!sameRepository || !nextChoices.some(choice => (choice.quantization ?? 'unknown') === quantization.value)) {
      quantization.value = nextChoices[0]?.quantization ?? (nextChoices.length ? 'unknown' : ''); candidate.value = '';
    }
    if (sameRepository) {
...
async function remove({ repository }: { repository: string }): Promise<void> {
  ...
  try {
    ...
    if (disposed || !await showConfirm({ message: await ensureStrings.llamaCppBrowser__delete_model_confirmation(), confirmButtonVariant: 'danger',
    ...
    <fieldset :disabled="disabled || active !== undefined || deleting" tw-class="space-y-5 disabled:opacity-50">
      ...
      <template v-if="catalog">
        <div tw-class="flex flex-wrap items-end gap-x-6 gap-y-4" data-testid="llama-hf-options">
          <div tw-class="w-36 max-w-full space-y-2">
            ...
            <div tw-class="relative">
              <select :id="`${id}-model`" v-model="quantization" data-testid="llama-hf-model" tw-class="appearance-none block w-full pl-3 pr-9 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm font-medium text-gray-800 dark:text-gray-100 focus:ring-4 focus:ring-purple-500/10 outline-none transition-all">
                ...
                <option v-for="choice in choices" :key="choice.quantization ?? 'unknown'" :value="choice.quantization ?? 'unknown'">{{ choice.quantization ?? lazyStrings.LlamaCppBrowserHuggingFaceManager__unspecified_quantization() }}</option>
              </select>
          ...
          <div tw-class="flex items-center gap-3 pb-3">
            <div>
        ...
        <p v-if="!catalog.projectors.length" tw-class="text-xs text-gray-500 dark:text-gray-400">{{ lazyStrings.LlamaCppBrowserHuggingFaceManager__no_companion_files_available() }}</p>
        ...
        <details ref="details" data-testid="llama-hf-details" tw-class="text-xs text-gray-500 dark:text-gray-400">
          ...
          <div tw-class="mt-3 space-y-4">
            ...
            <div v-if="multimodal === 'on' && catalog.projectors.length > 1" tw-class="space-y-2">
              <label :for="`${id}-projector`" tw-class="block font-bold">{{ lazyStrings.LlamaCppBrowserHuggingFaceManager__companion_file() }}</label>
              <select :id="`${id}-projector`" :value="selectedProjector?.path ?? ''" data-testid="llama-hf-projector" tw-class="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100" @change="projector = ($event.target as HTMLSelectElement).value">
                <option value="">{{ lazyStrings.LlamaCppBrowserHuggingFaceManager__choose_companion_file() }}</option>
                <option v-for="file in catalog.projectors" :key="file.path" :value="file.path">{{ file.path }} · {{ size({ bytes: file.size }) }}</option>
              </select>
            </div>
