...
describe('build-specific llama profile policy', () => {
  it('keeps auto and all four hosted profiles', () => {
    ...
    expect(hosted.selectableProfiles).toEqual(['auto', 'webgpu-wasm64-jspi', 'webgpu-wasm32-asyncify', 'cpu-wasm64', 'cpu-wasm32']);
  ...
  it.each(['auto', 'cpu-wasm32', 'cpu-wasm64', 'webgpu-wasm32-asyncify'] as const)('rejects %s rather than falling back in standalone', profile => {
