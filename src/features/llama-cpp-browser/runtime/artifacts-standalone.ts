...
import { decodeEmbeddedGzip } from '@/features/file-protocol-standalone/embedded-binary';
...
export async function loadWasmBinary({ profile, assetBaseURL }: { profile: LlamaCppProfile, assetBaseURL: string | undefined }): Promise<Uint8Array> {
  ...
  const { base64, byteLength } = await import('virtual:file-protocol-standalone/binary/llama-cpp-browser');
  return decodeEmbeddedGzip({ base64, byteLength });
}
