...
const cases = [
  {
    id: 'gemma-4-e2b', name: 'Gemma 4 E2B it (QAT)',
  ...
  {
    id: 'gemma-4-e4b', name: 'Gemma 4 E4B it (QAT)',
  ...
  {
    id: 'gemma-4-26b', name: 'Gemma 4 26B-A4B it (QAT)',
...
describe.each(cases)('Gemma QAT suggestion $id', fixture => {
  ...
  it('uses the official QAT source and refreshed size hints without changing memory tiers', () => {
    expect(suggestion).toEqual({
      id: fixture.id, name: fixture.name, developer: 'Google',
  ...
  it('resolves the real main filename and only adds the same-repository projector when requested', () => {
    ...
    expect(resolveSuggestionPlan({ suggestion, catalog, multimodal: 'off' })).toEqual({ repository: fixture.repository, revision: catalog.revision, files: [main] });
    expect(resolveSuggestionPlan({ suggestion, catalog, multimodal: 'on' })).toEqual({ repository: fixture.repository, revision: catalog.revision, files: [main, projector] });
  });
  ...
  it('does not treat a previous non-QAT installation or another repository as the QAT checkpoint', () => {
    ...
    expect(findLocalSuggestedModel({ suggestion, models: [previous, anotherSource] })).toBeUndefined();
    expect(findLocalSuggestedModel({ suggestion, models: [previous, local] })).toEqual(local);
    expect(localModelDisplayName({ model: local })).toBe(`${fixture.name} · Q4_0`);
    expect(localModelDisplayName({ model: previous })).toBe(previous.name);
    expect(() => resolveSuggestionPlan({ suggestion, catalog: { ...catalog, repository: fixture.previousRepository }, multimodal: 'off' })).toThrow(SuggestionPlanError);
  });
});
