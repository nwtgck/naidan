...
import { modelSuggestions, matchesMemoryHint, suggestedMemoryFilters, moreGgufModelsUrl } from './model-suggestions';
...
const suggestion = modelSuggestions.find(entry => entry.id === 'muse-glimmer-30b')!;
...
function catalog({ paths }: { paths: string[] }) {
  return { repository: suggestion.repository, revision, ...groupModelFiles({ files: paths.map(path => ({ path, size: 128 })) }) };
}
describe('bundled suggestions and exact download plans', () => {
  it('keeps the agreed developer order, small models and only the requested memory chips', () => {
    ...
    expect(matchesMemoryHint({ suggestion, memory: 16, multimodal: 'off' })).toBe(false);
    expect(matchesMemoryHint({ suggestion, memory: 32, multimodal: 'off' })).toBe(true);
    expect(matchesMemoryHint({ suggestion, memory: 'all', multimodal: 'on' })).toBe(true);
  });
  it('leads with complete quantization tokens while preserving stored variant names', () => {
    ...
    expect(variantLabel({ repository: suggestion.repository, path: main })).toBe('KQuant-17GB-Q4_K_M');
  });
  it('excludes auxiliary GGUFs and opt-in projectors without trusting size hints', () => {
    ...
    const changedHints = { ...suggestion, approximateModelBytes: 1, approximateMultimodalBytes: 2 };
    expect(resolveSuggestionPlan({ suggestion: changedHints, catalog: data, multimodal: 'off' })).toEqual({ repository: suggestion.repository, revision, files: [{ path: main, size: 128 }] });
    expect(resolveSuggestionPlan({ suggestion: changedHints, catalog: data, multimodal: 'on' }).files.map(file => file.path)).toEqual([main, projector]);
  ...
  it('includes complete split parts but refuses incomplete splits, ambiguity or a different scheme', () => {
    ...
    expect(resolveSuggestionPlan({ suggestion, catalog: catalog({ paths: parts }), multimodal: 'off' }).files.map(file => file.path)).toEqual(parts);
    for (const paths of [[parts[0]!], ['Model-Q4_K_XL.gguf'], [main, 'other-Q4_K_M.gguf']]) {
      expect(() => resolveSuggestionPlan({ suggestion, catalog: catalog({ paths }), multimodal: 'off' })).toThrow(SuggestionPlanError);
    }
    expect(() => resolveSuggestionPlan({ suggestion, catalog: catalog({ paths: [main] }), multimodal: 'on' })).toThrow(SuggestionPlanError);
    expect(() => resolveSuggestionPlan({ suggestion, catalog: catalog({ paths: [main, 'mmproj-one-F16.gguf', 'mmproj-two-Q8_0.gguf'] }), multimodal: 'on' })).toThrow(SuggestionPlanError);
  });
  it('matches local repository identity and the exact main quantization, not approximate bytes or a filename coincidence', () => {
    const local = { id: `hf.co/${suggestion.repository}:${encodeURIComponent(main)}`, name: 'already installed', size: 128, importedAt: 1 };
    expect(findLocalSuggestedModel({ suggestion, models: [local] })).toEqual(local);
    expect(findLocalSuggestedModel({ suggestion, models: [{ ...local, id: `user/${main}` }] })).toBeUndefined();
    for (const path of ['model-Q8_0.gguf', 'model-Q4_K_XL.gguf', 'dflash-Q4_K_M.gguf']) {
      expect(findLocalSuggestedModel({ suggestion, models: [{ ...local, id: `hf.co/${suggestion.repository}:${encodeURIComponent(path)}` }] })).toBeUndefined();
    }
    expect(findLocalSuggestedModel({ suggestion, models: [local, { ...local, id: `hf.co/${suggestion.repository}:other-Q4_K_M.gguf` }] })).toBeUndefined();
  });
  it('pins normal file-save links, encodes nested filenames and does not confuse decimal bytes with GiB', () => {
    ...
    expect(fileDownloadUrl({ repository: suggestion.repository, revision, file })).toBe(`https://huggingface.co/${suggestion.repository}/resolve/${revision}/nested/model%20name-Q4_K_M.gguf?download=true`);
    expect(() => fileDownloadUrl({ repository: suggestion.repository, revision: 'main', file })).toThrow();
    expect(() => fileDownloadUrl({ repository: suggestion.repository, revision, file: { ...file, path: '../bad.gguf' } })).toThrow();
    expect(formatDownloadBytes({ bytes: 16_800_000_000 })).toBe('15.65 GiB');
    const plan = resolveSuggestionPlan({ suggestion, catalog: catalog({ paths: [main, projector] }), multimodal: 'on' });
