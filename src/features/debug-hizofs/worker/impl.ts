/* eslint-disable local-rules/enforce-dependency-directions -- This read-only debug worker intentionally traverses exact persisted HizoFS DTOs so audits are not distorted by domain mapping. */
import type {
  HizoFSCommitDto,
  HizoFSDirectoryEntryDto,
  HizoFSDirectoryIndexPageDto,
  HizoFSDirectoryInodeDto,
  HizoFSFileExtentPageDto,
  HizoFSFileInodeDto,
...
import {
  ...
  type HizoFSInspectedObjectView,
  type HizoFSIntegrityScanResult,
  type HizoFSNamespaceResult,
  type HizoFSResolvedNodeView,
  type HizoFSResolvedPathView,
...
type LoadedObject = NonNullable<Awaited<ReturnType<HizoFSInspectionReader['inspectObject']>>>;

export function createHizoFSInspectionWorker(): IHizoFSInspectionWorker {
  let reader: HizoFSInspectionReader | undefined;
  let operationGeneration = 0;

  function requireReader(): HizoFSInspectionReader {
    if (reader === undefined) {
      throw new Error('HizoFS inspection worker is not configured');
    }
    return reader;
  }
  ...
  function assertOperationActive({ generation }: { generation: number }): void {
    if (generation !== operationGeneration) {
      throw new DOMException('HizoFS inspection operation was cancelled', 'AbortError');
    }
  }
  ...
  async function readResolvedNode({
    ...
    if (
      ...
      throw new Error('HizoFS directory traversal limit is invalid');
    }
    ...
    if (commitObject === undefined) {
      throw new Error(`HizoFS commit object is missing: ${commitObjectId}`);
    }
    ...
    if (inodeObject === undefined) {
      throw new Error(`HizoFS inode object is missing: ${lookup.inodeObjectId}`);
    }

    let inodeKind: HizoFSResolvedNodeView['inodeKind'];
    let inodePersistedDto: unknown;
    let directory: HizoFSResolvedNodeView['directory'];
    ...
    case 'file_inode': {
      ...
      if (inode.nodeId !== nodeId) {
        throw new Error(`HizoFS file inode nodeId does not match lookup: ${nodeId}`);
      }
      inodeKind = 'file';
    ...
    case 'directory_inode': {
      ...
      if (inode.nodeId !== nodeId) {
        throw new Error(`HizoFS directory inode nodeId does not match lookup: ${nodeId}`);
      }
      inodeKind = 'directory';
      inodePersistedDto = inodeObject.record.metadata;
    ...
    case 'symlink_inode': {
      ...
      if (inode.nodeId !== nodeId) {
        throw new Error(`HizoFS symlink inode nodeId does not match lookup: ${nodeId}`);
      }
      inodeKind = 'symlink';
      inodePersistedDto = inodeObject.record.metadata;
      directory = undefined;
    ...
    default:
      throw new Error(`Unhandled HizoFS record kind: ${inodeObject.record.kind}`);
    }
  ...
  async function resolveInodeIndexLookup({ rootObjectId, nodeId }: {
    ...
    readonly inodeObjectId: string;
    readonly steps: HizoFSResolvedNodeView['inodeIndexLookup'];
  }> {
    const steps: Array<HizoFSResolvedNodeView['inodeIndexLookup'][number]> = [];
    ...
    while (true) {
      ...
      if (object === undefined) {
        throw new Error(`HizoFS inode index page is missing: ${pageObjectId}`);
      }
      ...
      switch (page.type) {
      case 'leaf': {
        const entry = page.entries.find(candidate => compareHizoFSStrings({
          left: candidate.nodeId,
          right: nodeId,
        }) === 0);
        if (entry === undefined) {
          throw new Error(`HizoFS inode index does not contain node: ${nodeId}`);
        }
        steps.push({
          type: 'leaf',
          pageObjectId,
      ...
      case 'branch': {
        const child = page.children.find(candidate => compareHizoFSStrings({
          left: nodeId,
          right: candidate.upperBoundNodeId,
        }) <= 0);
        if (child === undefined) {
          throw new Error(`HizoFS inode index branch does not cover node: ${nodeId}`);
        }
        steps.push({
          type: 'branch',
          pageObjectId,
  ...
  async function readDirectoryEntriesForTraversal({
    ...
    maximumDirectoryEntryCount: number;
  }): Promise<NonNullable<HizoFSResolvedNodeView['directory']>> {
    ...
    case 'indexed': {
      const entries: NonNullable<HizoFSResolvedNodeView['directory']>['entries'][number][] = [];
      const issues: string[] = [];
      const visited = new Set<string>();
      let truncated = false;
  ...
  async function resolveDirectoryEntryForTraversal({
    ...
    directoryInodeObjectId: string;
    name: string;
  }): Promise<NonNullable<HizoFSResolvedNodeView['directory']>['entries'][number]> {
    switch (inode.storage.type) {
    case 'inline': {
      const entry = inode.storage.entries.find(candidate => compareHizoFSStrings({
        left: candidate.name,
        right: name,
      }) === 0);
      if (entry === undefined) {
        throw new Error(`HizoFS directory entry is missing: ${name}`);
      }
      return {
        entry,
        source: {
    ...
    case 'indexed': {
      ...
      while (true) {
        ...
        if (object === undefined) {
          throw new Error(`HizoFS directory index page is missing: ${pageObjectId}`);
        }
        if (object.record.kind !== 'directory_index_page') {
        ...
        case 'branch': {
          ...
          if (child === undefined) {
            throw new Error(`HizoFS directory index branch does not cover entry: ${name}`);
          }
          pageObjectId = child.childPageObjectId;
          break;
        }
  ...
  async function readResolvedPath({
    ...
    if (!logicalPath.startsWith('/')) {
      throw new Error('HizoFS logical path must be absolute');
    }
    const pathSegments = logicalPath === '/'
      ? []
      : logicalPath.slice(1).split('/');
    if (pathSegments.some(segment => segment.length === 0 || segment === '.' || segment === '..')) {
      throw new Error(`HizoFS logical path is not normalized: ${logicalPath}`);
    }
    ...
    const resolvedNodes: HizoFSResolvedPathView[number][] = [];
    ...
    for (let index = 0; index <= pathSegments.length; index += 1) {
      ...
      if (resolved.inodeKind !== 'directory' || resolved.directory === undefined) {
        throw new Error(`HizoFS path enters a non-directory node: ${currentPath}`);
      }
      ...
      if (!resolved.directory.entries.some(candidate => compareHizoFSStrings({
  ...
  return {
    ...
    async readOverview() {
      return hizoFSInspectionOverviewSchema.parse(await requireReader().readOverview());
    },
    ...
    async readNamespace({ maximumEntryCount }) {
      if (!Number.isSafeInteger(maximumEntryCount) || maximumEntryCount < 1 || maximumEntryCount > 100_000) {
        throw new Error('HizoFS namespace inspection limit is invalid');
      }
      const operation = beginOperation();
      const overview = hizoFSInspectionOverviewSchema.parse(await requireReader().readOverview());
      const issues: string[] = [];
      ...
      const entries: Array<HizoFSNamespaceResult['entries'][number]> = [];
    ...
    async runIntegrityScan() {
      ...
      const inspectedCache = new Map<string, HizoFSInspectedObjectView | undefined>();
      ...
      do {
        assertOperationActive(operation);
        const page = hizoFSPhysicalObjectPageSchema.parse(
  ...
  async function readDirectoryEntries({
    ...
    operation: { readonly generation: number };
    issues: string[];
  }): Promise<readonly HizoFSDirectoryEntryDto[]> {
    switch (inode.storage.type) {
    case 'inline':
...
function cloneInspectedObject({ object }: { object: LoadedObject }): HizoFSInspectedObjectView['object'] {
...
function parsePersistedDto({ object }: {
  ...
  readonly validation: HizoFSInspectedObjectView['validation'];
  readonly references: HizoFSInspectedObjectView['references'];
  readonly rootDirectoryEntryPoint: HizoFSInspectedObjectView['rootDirectoryEntryPoint'];
} {
  try {
...
function getPersistedDtoSchema({ kind }: { kind: string }) {
  ...
  default:
    throw new Error(`Unsupported HizoFS record kind: ${kind}`);
  }
}
...
function deriveReferences({ kind, persistedDto }: {
...
}): HizoFSInspectedObjectView['references'] {
  switch (kind) {
  case 'commit': {
    const dto = persistedDto as HizoFSCommitDto;
    return [{ relation: 'inode index root', objectId: dto.inodeIndexRootObjectId }];
  }
  case 'inode_index_page': {
    const dto = persistedDto as HizoFSInodeIndexPageDto;
    switch (dto.type) {
    case 'leaf':
  ...
  case 'file_inode': {
    const dto = persistedDto as HizoFSFileInodeDto;
  ...
  case 'directory_inode': {
    const dto = persistedDto as HizoFSDirectoryInodeDto;
    switch (dto.storage.type) {
    case 'inline': return [];
  ...
  case 'file_extent_page': {
    const dto = persistedDto as HizoFSFileExtentPageDto;
    switch (dto.type) {
    case 'leaf':
  ...
  case 'directory_index_page': {
    const dto = persistedDto as HizoFSDirectoryIndexPageDto;
    switch (dto.type) {
    case 'leaf': return [];
    case 'branch':
