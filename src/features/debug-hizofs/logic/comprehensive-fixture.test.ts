...
describe('HizoFS comprehensive fixture', () => {
  it('generates every persisted record family and crosses each default index boundary', async () => {
    ...
    const session = await createHizoFS({ backingDirectory, fileSystemRootKey: rootKey });
  ...
  it('refuses to merge a second fixture into the same root', async () => {
    ...
    try {
      await generateHizoFSComprehensiveFixture({
        root: session.root,
