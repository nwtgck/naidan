...
import {
  createHizoFSWorkbenchWorkspace,
  destroyHizoFSWorkbenchWorkspace,
  listHizoFSWorkbenchSources,
  openHizoFSWorkbenchSource,
  type HizoFSWorkbenchSource,
  type HizoFSWorkbenchSourceSession,
...
import {
  ...
  type HizoFSComprehensiveFixtureProgress,
  type HizoFSComprehensiveFixtureResult,
} from '@/features/debug-hizofs/logic/comprehensive-fixture';
import { createHizoFSInspectionWorkerClient } from '@/features/debug-hizofs/worker/client';
import type {
  HizoFSBinaryRecordInspectionView,
  HizoFSBinarySliceView,
  HizoFSInspectionOverviewView,
  HizoFSInspectionWorkerClient,
  HizoFSInspectedObjectView,
  HizoFSIntegrityScanResult,
  HizoFSNamespaceResult,
  HizoFSPhysicalObjectPageView,
 ...
 * mistaken for the HizoFS persistence protocol itself.
 */
type WorkbenchDetailColumn =
  ...
  | {
      ...
      loading: boolean;
      value: HizoFSInspectedObjectView | undefined;
  ...
  | {
      ...
      loading: boolean;
      value: HizoFSResolvedNodeView | undefined;
      errorMessage: string | undefined;
    }
  | {
...
const { closeDebugHizoFSWorkbench } = useDebugHizoFSWorkbench();
const { openDebugOpfsEncryptionInspector } = useDebugOpfsEncryptionInspector();
const { openFileExplorer } = useFileExplorerModal();

const sources = ref<readonly HizoFSWorkbenchSource[]>([]);
const selectedSource = ref<HizoFSWorkbenchSource>();
const sourceSession = ref<HizoFSWorkbenchSourceSession>();
const client = ref<HizoFSInspectionWorkerClient>();
const overview = ref<HizoFSInspectionOverviewView>();
...
const comprehensiveFixtureProgress = ref<HizoFSComprehensiveFixtureProgress>();
const comprehensiveFixtureResult = ref<HizoFSComprehensiveFixtureResult>();
const comprehensiveFixtureErrorMessage = ref<string>();
const errorMessage = ref<string>();

const namespaceLoading = ref(false);
const namespaceResult = ref<HizoFSNamespaceResult>();
const objectsLoading = ref(false);
const objectPage = ref<HizoFSPhysicalObjectPageView>();
const objectEntries = ref<HizoFSPhysicalObjectPageView['entries']>([]);
const objectScrollTop = ref(0);
const objectViewportHeight = ref(520);
const integrityLoading = ref(false);
const integrityResult = ref<HizoFSIntegrityScanResult>();
type SuperblockSlotInspectionView = Awaited<ReturnType<HizoFSInspectionWorkerClient['inspectSuperblockSlot']>>;
 ...
 * losing the context of which HizoFS it belongs to.
 */
...
const fileExplorerRoot = computed<FileExplorerRootDescriptor | undefined>(() => {
  ...
  /**
   * The decrypted HizoFS root is passed directly to File Explorer.
   *
...
async function refreshSources({ preferredSourceId }: {
  ...
  try {
    const nextSources = await listHizoFSWorkbenchSources();
...
async function createWorkspace(): Promise<void> {
  ...
  errorMessage.value = undefined;
  try {
    const source = await createHizoFSWorkbenchWorkspace();
...
async function destroyWorkspace({ source }: {
  ...
  try {
    ...
    await destroyHizoFSWorkbenchWorkspace({ source });
    await refreshSources({ preferredSourceId: undefined });
  } catch (error) {
    errorMessage.value = toErrorMessage({ error });
  }
...
async function selectSource({ source, force = false }: {
  ...
  try {
    const nextSession = await openHizoFSWorkbenchSource({ source });
    const nextClient = await createHizoFSInspectionWorkerClient({
      reader: nextSession.hizoFSReader,
    });
 ...
 * superblock transition must be produced by normal HizoFS operations.
 */
...
async function openRootDirectoryEntryPoint({
  ...
  entryPoint: NonNullable<HizoFSInspectedObjectView['rootDirectoryEntryPoint']>;
  afterIndex: number;
}): Promise<void> {
  await openResolvedNode({
...
async function openResolvedNode({
  ...
  if (currentClient === undefined) {
    ...
    column.errorMessage = 'No HizoFS inspection worker is open';
    return;
  }
  try {
...
function getDirectoryEntrySourceObjectId({
  ...
  source: NonNullable<HizoFSResolvedNodeView['directory']>['entries'][number]['source'];
}): string {
  switch (source.type) {
  case 'inline':
...
function getObjectPersistedDto({ value }: {
  value: HizoFSInspectedObjectView;
}): unknown {
...
<template>
  <Teleport to="body">
    <div tw-class="fixed inset-0 z-[125] flex items-center justify-center bg-black/55 p-2 sm:p-4" @click.self="closeDebugHizoFSWorkbench">
        ...
        <div v-if="primaryView === 'inspection'" data-workbench-column-scroll tw-class="flex min-h-0 flex-1 overflow-x-auto overflow-y-hidden bg-gray-100 dark:bg-gray-950">
          <aside tw-class="flex w-[300px] shrink-0 flex-col border-r border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
            ...
            <div v-else tw-class="min-h-0 flex-1 overflow-auto">
              <div v-if="sources.length === 0" tw-class="p-4 text-xs text-gray-500">No HizoFS instance is open. Create an ephemeral workspace to inspect the filesystem before enabling Naidan encryption.</div>
              <div v-for="source in sources" :key="source.sourceId" :tw-class="['group flex border-b border-gray-100 dark:border-gray-800', selectedSource?.sourceId === source.sourceId ? 'bg-emerald-50 dark:bg-emerald-950/30' : 'bg-white dark:bg-gray-900']">
                <button type="button" data-testid="hizofs-source" :data-source-id="source.sourceId" tw-class="min-w-0 flex-1 px-3 py-3 text-left hover:bg-gray-50 disabled:opacity-50 dark:hover:bg-gray-800" :disabled="generatingComprehensiveFixture" @click="selectSource({ source })">
            ...
            <div v-else-if="overview" tw-class="min-h-0 flex-1 overflow-auto">
              ...
              <section v-if="selectedSource.type === 'ephemeral_debug_workspace'" tw-class="border-b border-amber-200 bg-amber-50/60 dark:border-amber-900 dark:bg-amber-950/10">
                ...
                <button type="button" data-testid="hizofs-open-comprehensive-fixture" tw-class="flex w-full items-center gap-3 px-3 py-3 text-left hover:bg-amber-100/70 disabled:opacity-50 dark:hover:bg-amber-950/30" :disabled="generatingComprehensiveFixture" @click="openComprehensiveFixturePanel">
                  ...
                  <span tw-class="min-w-0 flex-1"><span tw-class="block text-xs font-medium">Generate comprehensive fixture…</span><span tw-class="block text-[9px] text-amber-700/80 dark:text-amber-300/80">Create deterministic coverage for every current HizoFS record family</span></span>
                  <ChevronRightIcon tw-class="h-3.5 w-3.5 text-amber-500" />
                </button>
                <div v-if="fixturePanelVisibility === 'open'" data-testid="hizofs-comprehensive-fixture-panel" tw-class="border-t border-amber-200 px-3 py-3 text-[10px] text-amber-950 dark:border-amber-900 dark:text-amber-100">
                  <p>Creates a deterministic dataset under <code tw-class="font-mono">{{ HIZOFS_COMPREHENSIVE_FIXTURE_ROOT_PATH }}</code> through the decrypted filesystem API.</p>
                  ...
                  <div v-if="comprehensiveFixtureProgress" data-testid="hizofs-comprehensive-fixture-progress" tw-class="mt-3 rounded border border-amber-300 bg-white/70 p-2 font-mono text-[9px] dark:border-amber-800 dark:bg-gray-950/50">
                  ...
                  <div v-if="comprehensiveFixtureErrorMessage" data-testid="hizofs-comprehensive-fixture-error" tw-class="mt-3 rounded border border-red-300 bg-red-50 p-2 text-red-700 dark:border-red-900 dark:bg-red-950/30 dark:text-red-300">{{ comprehensiveFixtureErrorMessage }}</div>
                  <div v-if="comprehensiveFixtureResult" data-testid="hizofs-comprehensive-fixture-result" tw-class="mt-3 rounded border border-emerald-300 bg-emerald-50 p-2 text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-300">
                  ...
                  <div tw-class="mt-3 flex flex-wrap justify-end gap-2">
                    <button v-if="comprehensiveFixtureResult" type="button" data-testid="hizofs-inspect-comprehensive-fixture-root" tw-class="rounded border border-emerald-400 px-2 py-1 font-medium text-emerald-700 hover:bg-emerald-100 dark:border-emerald-800 dark:text-emerald-300 dark:hover:bg-emerald-950/40" @click="inspectLogicalPath({ logicalPath: comprehensiveFixtureResult.rootPath })">Inspect fixture root</button>
                    ...
                    <button type="button" data-testid="hizofs-generate-comprehensive-fixture" tw-class="rounded border border-amber-500 bg-amber-600 px-2 py-1 font-medium text-white hover:bg-amber-700 disabled:opacity-50" :disabled="generatingComprehensiveFixture || comprehensiveFixtureResult !== undefined" @click="generateComprehensiveFixture">{{ generatingComprehensiveFixture ? 'Generating…' : 'Generate' }}</button>
                  </div>
                </div>
              </section>
              <button type="button" data-testid="hizofs-open-descriptor" tw-class="flex w-full items-center gap-3 border-b border-gray-100 px-3 py-3 text-left hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800" @click="openDescriptor"><FileCode2Icon tw-class="h-4 w-4 text-emerald-600" /><span tw-class="min-w-0 flex-1"><span tw-class="block text-xs font-medium">Descriptor</span><span tw-class="block text-[9px] text-gray-400">RAW DTO · persisted plaintext</span></span><ChevronRightIcon tw-class="h-3.5 w-3.5 text-gray-400" /></button>
              <button type="button" data-testid="hizofs-open-superblocks" tw-class="flex w-full items-center gap-3 border-b border-gray-100 px-3 py-3 text-left hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800" @click="openSuperblockSlots"><HardDriveIcon tw-class="h-4 w-4 text-emerald-600" /><span tw-class="min-w-0 flex-1"><span tw-class="block text-xs font-medium">Superblock slots</span><span tw-class="block text-[9px] text-gray-400">PERSISTED RECORDS · A/B selection</span></span><ChevronRightIcon tw-class="h-3.5 w-3.5 text-gray-400" /></button>
              ...
              <button type="button" data-testid="hizofs-open-root-directory" tw-class="flex w-full items-center gap-3 border-b border-gray-100 px-3 py-3 text-left hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800" @click="openActiveRootDirectory({ afterIndex: -1 })"><FolderRootIcon tw-class="h-4 w-4 text-emerald-600" /><span tw-class="min-w-0 flex-1"><span tw-class="block text-xs font-medium">Root directory</span><span tw-class="block text-[9px] text-gray-400">SHORTCUT · commit.rootDirectoryNodeId → inode index → inode</span></span><ChevronRightIcon tw-class="h-3.5 w-3.5 text-gray-400" /></button>
              <button type="button" data-testid="hizofs-open-object-store" tw-class="flex w-full items-center gap-3 border-b border-gray-100 px-3 py-3 text-left hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800" @click="openObjectStore"><BoxesIcon tw-class="h-4 w-4 text-emerald-600" /><span tw-class="min-w-0 flex-1"><span tw-class="block text-xs font-medium">Physical object store</span><span tw-class="block text-[9px] text-gray-400">PERSISTED OBJECTS · cursor paged</span></span><ChevronRightIcon tw-class="h-3.5 w-3.5 text-gray-400" /></button>
              <button type="button" data-testid="hizofs-open-integrity" tw-class="flex w-full items-center gap-3 border-b border-gray-100 px-3 py-3 text-left hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800" @click="openIntegrity"><ShieldCheckIcon tw-class="h-4 w-4 text-emerald-600" /><span tw-class="min-w-0 flex-1"><span tw-class="block text-xs font-medium">Integrity and reachability</span><span tw-class="block text-[9px] text-gray-400">DERIVED FROM PERSISTED REFERENCES</span></span><ChevronRightIcon tw-class="h-3.5 w-3.5 text-gray-400" /></button>
          ...
          <section v-for="(column, columnIndex) in columns" :key="column.id" tw-class="flex w-[440px] shrink-0 flex-col border-r border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
            ...
            <div v-else-if="column.kind === 'superblock_slots'" tw-class="min-h-0 flex-1 overflow-auto">
              <button v-for="(slot, slotIndex) in overview?.superblockSlots ?? []" :key="String(slot.slot)" type="button" data-testid="hizofs-superblock-slot" tw-class="block w-full border-b border-gray-100 px-3 py-3 text-left hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800" @click="openSuperblockSlot({ slotIndex, afterIndex: columnIndex })">
            ...
            <div v-else-if="column.kind === 'active_commit'" tw-class="min-h-0 flex-1 overflow-auto">
              ...
              <div v-if="overview" tw-class="m-3 space-y-2">
                ...
                <button type="button" data-testid="hizofs-open-root-from-commit" tw-class="flex w-full items-center justify-between rounded border border-emerald-300 px-3 py-2 text-left text-xs text-emerald-700 hover:bg-emerald-50 dark:border-emerald-800 dark:text-emerald-300 dark:hover:bg-emerald-950/30" @click="openActiveRootDirectory({ afterIndex: columnIndex })"><span>Resolve commit.rootDirectoryNodeId</span><ChevronRightIcon tw-class="h-3.5 w-3.5" /></button>
              </div>
            </div>
            ...
            <div v-else-if="column.kind === 'object_store'" tw-class="flex min-h-0 flex-1 flex-col">
              ...
              <div tw-class="min-h-0 flex-1 overflow-auto" @scroll="handleObjectListScroll({ event: $event })">
                ...
                <button v-for="entry in visibleObjectEntries" :key="entry.objectId" type="button" :style="{ height: `${String(OBJECT_ROW_HEIGHT)}px` }" data-testid="hizofs-object-entry" tw-class="block w-full border-b border-gray-100 px-3 py-2 text-left hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800" @click="openObject({ objectId: entry.objectId, relation: 'Object record', afterIndex: columnIndex })">
                ...
                >
                  <template #references>
                    <section v-if="column.value.rootDirectoryEntryPoint || column.value.references.length > 0" tw-class="border-t border-gray-200 p-3 dark:border-gray-700">
                      ...
                      <button v-if="column.value.rootDirectoryEntryPoint" type="button" data-testid="hizofs-open-root-from-object" tw-class="mb-1 flex w-full items-center justify-between rounded border border-emerald-300 px-3 py-2 text-left text-xs text-emerald-700 hover:bg-emerald-50 dark:border-emerald-800 dark:text-emerald-300 dark:hover:bg-emerald-950/30" @click="openRootDirectoryEntryPoint({ entryPoint: column.value.rootDirectoryEntryPoint, afterIndex: columnIndex })"><span>Resolve rootDirectoryNodeId through inode index</span><ChevronRightIcon tw-class="h-3.5 w-3.5" /></button>
            ...
            <div v-else-if="column.kind === 'resolved_node'" tw-class="min-h-0 flex-1 overflow-auto">
              ...
              <template v-else-if="column.value">
                ...
                <div tw-class="border-b border-gray-200 p-3 dark:border-gray-700">
                  ...
                  <button type="button" data-testid="hizofs-open-resolved-inode" tw-class="flex w-full items-center justify-between gap-2 rounded border border-emerald-300 px-2 py-1.5 text-left text-emerald-700 hover:bg-emerald-50 dark:border-emerald-800 dark:text-emerald-300 dark:hover:bg-emerald-950/30" @click="openObject({ objectId: column.value.inodeObjectId, relation: `${column.value.inodeKind} inode`, afterIndex: columnIndex })"><span tw-class="text-[9px]">resolved inode object</span><span tw-class="truncate font-mono text-[9px]">{{ column.value.inodeObjectId }}</span></button>
                </div>
            ...
            <div v-else-if="column.kind === 'derived_views'" tw-class="min-h-0 flex-1 overflow-auto">
              ...
              <button type="button" data-testid="hizofs-open-derived-file-explorer" tw-class="flex w-full items-center gap-3 border-b border-gray-100 px-3 py-3 text-left hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800" @click="openFileExplorerColumn({ afterIndex: columnIndex })"><FolderTreeIcon tw-class="h-4 w-4 text-blue-500" /><span tw-class="min-w-0 flex-1"><span tw-class="block text-xs font-medium">Derived filesystem view</span><span tw-class="block text-[9px] text-gray-400">Direct decrypted root · {{ selectedSource?.access }}</span></span><ChevronRightIcon tw-class="h-3.5 w-3.5" /></button>
            </div>
            ...
            <div v-else-if="column.kind === 'integrity'" tw-class="min-h-0 flex-1 overflow-auto">
              <div tw-class="flex items-center gap-2 border-b border-gray-200 p-3 dark:border-gray-700">
                <button type="button" data-testid="hizofs-run-integrity" tw-class="rounded bg-emerald-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-emerald-700 disabled:opacity-50" :disabled="integrityLoading" @click="runIntegrityScan">Run scan</button>
        ...
        <section v-if="primaryView === 'inspection' && selectedSource && fileExplorerRoot" data-testid="hizofs-companion-explorer" tw-class="shrink-0 border-t border-blue-200 bg-white dark:border-blue-900 dark:bg-gray-900">
          <header tw-class="flex items-center gap-3 px-3 py-2">
            <button type="button" data-testid="hizofs-toggle-companion-explorer" tw-class="flex min-w-0 flex-1 items-center gap-2 text-left" @click="toggleCompanionExplorer">
              ...
              <FilesIcon data-testid="hizofs-companion-files-icon" tw-class="h-4 w-4 shrink-0 text-blue-500" />
