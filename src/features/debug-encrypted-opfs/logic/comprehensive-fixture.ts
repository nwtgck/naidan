import {
  DEFAULT_ENCRYPTED_OPFS_POLICY,
  type EncryptedOpfsPolicy,
...
const FIXTURE_ROOT_NAME = '__encrypted_opfs_fixture__';
const INDEX_BRANCH_ENTRY_COUNT = DEFAULT_ENCRYPTED_OPFS_POLICY.indexPageEntryLimit + 1;
const DEEP_DIRECTORY_LEVEL_COUNT = 12;

export const ENCRYPTED_OPFS_COMPREHENSIVE_FIXTURE_ROOT_PATH = `/${FIXTURE_ROOT_NAME}`;

export type EncryptedOpfsComprehensiveFixtureProgress = {
  readonly phase:
    | 'preparing'
    | 'directories'
    | 'files'
...
export type EncryptedOpfsComprehensiveFixtureCase = {
  readonly id: string;
...
export type EncryptedOpfsComprehensiveFixtureResult = {
  readonly rootPath: string;
  readonly manifestPath: string;
  readonly coverage: readonly EncryptedOpfsComprehensiveFixtureCase[];
};

/**
 * Builds a deterministic fixture through the public decrypted filesystem API.
 *
 * The generator intentionally does not write EncryptedOpfs records directly.
 * Every inode, index page, chunk, commit, and superblock transition must be a
 ...
 *
 * Policy thresholds are read from the EncryptedOpfs default policy so the
 * fixture continues to cross the actual inline/indexed and inline/extent
 * boundaries when those defaults evolve.
 */
export async function generateEncryptedOpfsComprehensiveFixture({
  root,
  onProgress,
}: {
  root: StorageDirectoryHandle;
  onProgress: ({ progress }: {
    progress: EncryptedOpfsComprehensiveFixtureProgress;
  }) => void;
}): Promise<EncryptedOpfsComprehensiveFixtureResult> {
  ...
  onProgress({
    progress: {
      ...
      totalPhaseCount,
      detail: `Checking ${ENCRYPTED_OPFS_COMPREHENSIVE_FIXTURE_ROOT_PATH}`,
    },
  });
  await assertFixtureRootDoesNotExist({ root });

  const policy = DEFAULT_ENCRYPTED_OPFS_POLICY;
  ...
  const manifestPath = `${ENCRYPTED_OPFS_COMPREHENSIVE_FIXTURE_ROOT_PATH}/manifest.json`;
  await writeStorageFileText({
    fileHandle: await fixtureRoot.getFileHandle({ name: 'manifest.json', create: true }),
    value: JSON.stringify({
      fixtureVersion: 1,
      purpose: 'EncryptedOpfs persisted data-structure coverage',
      policy,
      coverage,
    }, undefined, 2),
  });
  ...
  return {
    rootPath: ENCRYPTED_OPFS_COMPREHENSIVE_FIXTURE_ROOT_PATH,
    manifestPath,
    coverage,
  };
}
...
async function createFileCoverage({ filesDirectory, policy }: {
  ...
  await writeFileBytes({
    ...
    bytes: new TextEncoder().encode('EncryptedOpfs inline fixture'),
  });
  await writeFileBytes({
...
async function createIndexCoverage({ indexedDirectory, filesDirectory, policy }: {
  ...
  for (let index = policy.inlineDirectoryEntryLimit + 2; index < INDEX_BRANCH_ENTRY_COUNT; index += 1) {
...
async function createSymlinkCoverage({ linksDirectory }: {
  ...
  await linksDirectory.createSymlink({
    ...
    target: `${ENCRYPTED_OPFS_COMPREHENSIVE_FIXTURE_ROOT_PATH}/directories/indexed`,
  });
  await linksDirectory.createSymlink({
    name: 'dangling',
...
function createCoverageCases({ policy }: {
  policy: EncryptedOpfsPolicy;
}): readonly EncryptedOpfsComprehensiveFixtureCase[] {
  return [
    {
      id: 'empty-directory',
      path: `${ENCRYPTED_OPFS_COMPREHENSIVE_FIXTURE_ROOT_PATH}/directories/empty`,
      purpose: 'Empty inline directory inode',
      expectedStructures: ['directory_inode:inline'],
    },
    {
      id: 'deep-directory',
      path: `${ENCRYPTED_OPFS_COMPREHENSIVE_FIXTURE_ROOT_PATH}/directories/deep`,
      purpose: 'Deep logical directory traversal',
      expectedStructures: ['directory_inode:inline', 'inode_index_page'],
    },
    {
      id: 'indexed-directory',
      path: `${ENCRYPTED_OPFS_COMPREHENSIVE_FIXTURE_ROOT_PATH}/directories/indexed`,
      purpose: `Directory with more than ${String(policy.inlineDirectoryEntryLimit)} entries`,
      expectedStructures: ['directory_inode:indexed', 'directory_index_page:leaf', 'directory_index_page:branch'],
    },
    {
      id: 'inline-file',
      path: `${ENCRYPTED_OPFS_COMPREHENSIVE_FIXTURE_ROOT_PATH}/files/inline-small.txt`,
      purpose: 'Inline file inode and binary payload',
      expectedStructures: ['file_inode:inline'],
    },
    {
      id: 'inline-boundary',
      path: `${ENCRYPTED_OPFS_COMPREHENSIVE_FIXTURE_ROOT_PATH}/files/inline-boundary.bin`,
      purpose: `Inline file at the ${String(policy.inlineFileByteLimit)} byte boundary`,
      expectedStructures: ['file_inode:inline'],
    },
    {
      id: 'extent-file',
      path: `${ENCRYPTED_OPFS_COMPREHENSIVE_FIXTURE_ROOT_PATH}/files/extent-single-chunk.bin`,
      purpose: 'File immediately above the inline boundary',
      expectedStructures: ['file_inode:extents', 'file_extent_page:leaf', 'file_chunk'],
    },
    {
      id: 'extent-index-branch',
      path: `${ENCRYPTED_OPFS_COMPREHENSIVE_FIXTURE_ROOT_PATH}/files/extent-index-branch.bin`,
    ...
    {
      id: 'sparse-file',
      path: `${ENCRYPTED_OPFS_COMPREHENSIVE_FIXTURE_ROOT_PATH}/files/sparse.bin`,
      purpose: 'Missing extent entries representing zero-filled sparse ranges',
      expectedStructures: ['file_inode:extents', 'file_extent_page:leaf', 'file_chunk'],
    },
    {
      id: 'symlinks',
      path: `${ENCRYPTED_OPFS_COMPREHENSIVE_FIXTURE_ROOT_PATH}/links`,
      purpose: 'Relative, absolute, directory, and dangling symlink inodes',
      expectedStructures: ['symlink_inode'],
    },
    {
      id: 'copy-on-write-history',
      path: `${ENCRYPTED_OPFS_COMPREHENSIVE_FIXTURE_ROOT_PATH}/operations`,
      purpose: 'Partial writes, extent-to-inline truncation, move, rename, and deletion history',
      expectedStructures: ['commit', 'superblock', 'unreachable immutable objects'],
    },
    {
      id: 'inode-index-branch',
      path: ENCRYPTED_OPFS_COMPREHENSIVE_FIXTURE_ROOT_PATH,
...
async function assertFixtureRootDoesNotExist({ root }: {
  ...
  throw new Error(
    `${ENCRYPTED_OPFS_COMPREHENSIVE_FIXTURE_ROOT_PATH} already exists. Remove it or create a new ephemeral workspace before generating the fixture.`,
  );
}
