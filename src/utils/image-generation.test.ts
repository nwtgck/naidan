...
describe('image-generation utilities', () => {
  describe('getImageGenerationModels', () => {
    ...
    it('should return empty array if no matches', () => {
      expect(getImageGenerationModels({ models: ['llama3', 'gpt-4'] })).toEqual([]);
    });
  });
  ...
  describe('isImageRequest', () => {
    it('should return true if content contains the request prefix', () => {
      expect(isImageRequest({ content: 'some text <!-- naidan_experimental_image_request {"width":512} --> more text' })).toBe(true);
    });

    it('should return false otherwise', () => {
      expect(isImageRequest({ content: 'just regular text' })).toBe(false);
    });
  });
  ...
  describe('parseImageRequest', () => {
    ...
    it('should return null for invalid format', () => {
      expect(parseImageRequest({ content: 'not a marker' })).toBeNull();
    });
  ...
  describe('status checks', () => {
    it('isImageGenerationPending should identify pending sentinel', () => {
      expect(isImageGenerationPending({ content: SENTINEL_IMAGE_PENDING })).toBe(true);
      expect(isImageGenerationPending({ content: 'no' })).toBe(false);
    });

    it('isImageGenerationProcessed should identify processed sentinel', () => {
      expect(isImageGenerationProcessed({ content: SENTINEL_IMAGE_PROCESSED })).toBe(true);
      expect(isImageGenerationProcessed({ content: 'no' })).toBe(false);
    });
  });
});
