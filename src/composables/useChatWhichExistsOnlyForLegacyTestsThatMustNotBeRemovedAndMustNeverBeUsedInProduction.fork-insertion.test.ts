...
describe('useChatWhichExistsOnlyForLegacyTestsThatMustNotBeRemovedAndMustNeverBeUsedInProduction Fork Insertion Logic', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    const chat = useChatWhichExistsOnlyForLegacyTestsThatMustNotBeRemovedAndMustNeverBeUsedInProduction();
  ...
  it('should insert fork at the top of the chat block when not in a chat group', async () => {
    ...
    chat.rootItems.value = [
      ...
      { id: 'chat:a', type: 'chat', chat: { id: toChatId({ raw: 'a' }), title: 'A', updatedAt: 0, groupId: null } } as unknown as SidebarItem,
    ];
    ...
    await chat.forkChat({ messageId: idToRaw({ id: toMessageId({ raw: 'm1' }) }) });
    ...
    if (item2?.type === 'chat') {
      expect(item2.chat.id).toBe(toChatId({ raw: 'a' }));
    } else {
  ...
  it('should insert fork at the top of the chat group when parent is in a chat group', async () => {
    ...
    if (groupItem?.type === 'chat_group') {
      ...
      if (secondChat?.type === 'chat') {
        expect(secondChat.chat.id).toBe(toChatId({ raw: 'a' }));
      } else {
