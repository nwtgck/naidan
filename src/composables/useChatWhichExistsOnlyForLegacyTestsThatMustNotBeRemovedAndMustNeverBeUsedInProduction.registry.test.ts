...
describe('useChatWhichExistsOnlyForLegacyTestsThatMustNotBeRemovedAndMustNeverBeUsedInProduction Registry Lifecycle', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    chats.clear();
  ...
  it('should not leak newly created chats in liveChatRegistry after creation is complete', async () => {
    const { createNewChat, currentChat, openChat, unregisterLiveInstance, TEST_ONLY } = useChatWhichExistsOnlyForLegacyTestsThatMustNotBeRemovedAndMustNeverBeUsedInProduction();
