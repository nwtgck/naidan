...
describe('useChatWhichExistsOnlyForLegacyTestsThatMustNotBeRemovedAndMustNeverBeUsedInProduction.duplicateChatGroup', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('should create a new group with copied settings and modified name', async () => {
    const { duplicateChatGroup, rootItems } = useChatWhichExistsOnlyForLegacyTestsThatMustNotBeRemovedAndMustNeverBeUsedInProduction();
    ...
    // Inject mock data into rootItems (which is internal but exposed via sidebarItems/rootItems in useChatWhichExistsOnlyForLegacyTestsThatMustNotBeRemovedAndMustNeverBeUsedInProduction)
