...
describe('useChatWhichExistsOnlyForLegacyTestsThatMustNotBeRemovedAndMustNeverBeUsedInProduction Multi-Tab Integration Scenarios (BUG FINDING)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  /**
   * Regression coverage: a branch generated by another tab must not overwrite
   * the branch that was already added to the same message.
   */
  it('Scenario: Two tabs branching from the same message simultaneously', async () => {
    const chatStoreA = useChatWhichExistsOnlyForLegacyTestsThatMustNotBeRemovedAndMustNeverBeUsedInProduction();
    ...
    // Preserve the original reply and the branches added by both tabs.
