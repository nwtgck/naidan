...
function resetSharedStorage() {
  shared[STORAGE_KEY] = {
    chats: new Map(),
    groups: new Map(),
    settings: { endpoint: { type: 'openai', url: 'http://localhost' }, titleGeneration: 'disabled', defaultModelId: 'gpt-4' },
    hierarchy: { items: [] },
    listeners: new Set(),
  };
}
...
describe('useChatWhichExistsOnlyForLegacyTestsThatMustNotBeRemovedAndMustNeverBeUsedInProduction Comprehensive Cross-Tab Sync', () => {
  beforeEach(() => {
    resetSharedStorage();
  ...
  async function createTab() {
    ...
    const { useChatWhichExistsOnlyForLegacyTestsThatMustNotBeRemovedAndMustNeverBeUsedInProduction } = await import('./useChatWhichExistsOnlyForLegacyTestsThatMustNotBeRemovedAndMustNeverBeUsedInProduction');
    const store = useChatWhichExistsOnlyForLegacyTestsThatMustNotBeRemovedAndMustNeverBeUsedInProduction();
