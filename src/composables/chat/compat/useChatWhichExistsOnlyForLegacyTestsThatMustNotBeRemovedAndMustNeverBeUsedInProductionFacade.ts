...
// This test-only facade preserves the existing chat behavior tests after production
// responsibilities were split into focused composables.
// Those tests must be retained; this is not a temporary migration shim.
// Never use this facade in production. Put new features in an existing focused
// composable or a new focused composable.
// Only add behavior here when required to preserve the legacy chat API.
export function useChatWhichExistsOnlyForLegacyTestsThatMustNotBeRemovedAndMustNeverBeUsedInProduction() {
