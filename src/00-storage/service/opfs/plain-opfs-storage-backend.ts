...
import { HostVolumeDB } from './host-volume-db';
import type { OpfsSpecialFileSystemType } from './opfs-transition-backend';
import {
...
export class PlainOPFSStorageBackend extends IStorageProvider {
  ...
  private async migrateV1UploadedFilesToBinaryObjects(): Promise<void> {
    try {
      ...
      console.log(`[PlainOPFSStorageBackend] Starting migration: ${MIGRATION_V1_UPLOADED_FILES_TO_BINARY_OBJECTS}`);
        ...
        case 'file': {
          if (entry.name.endsWith('.json')) {
            try {
              ...
              console.warn(`[PlainOPFSStorageBackend] Skipping corrupted chat content file: ${entry.name}`, jsonErr);
            }
          }
          break;
        }
      ...
      console.log(`[PlainOPFSStorageBackend] Migration completed: ${MIGRATION_V1_UPLOADED_FILES_TO_BINARY_OBJECTS}`);
    } catch (e) {
      ...
      if (!isNotFound) {
        console.error(`[PlainOPFSStorageBackend] Migration failed: ${MIGRATION_V1_UPLOADED_FILES_TO_BINARY_OBJECTS}`, e);
        throw e;
      }
    }
  }
  ...
  async *listBinaryObjects(): AsyncIterable<BinaryObject> {
    ...
    try {
      ...
      console.error('[PlainOPFSStorageBackend] Failed to list binary objects', e);
    }
  }
  ...
  async dump(): Promise<StorageSnapshot> {
    ...
    const contentStream = async function* (this: PlainOPFSStorageBackend): AsyncGenerator<MigrationChunkDto> {
      // 1. Stream all chats
      ...
      try {
        ...
        for await (const shardEntry of baseDir.values()) {
          ...
          case 'directory': {
            ...
            for (const bId of Object.keys(index.objects)) {
              ...
              try {
                const source: StorageBinaryObjectWriteSource = (() => {
                  switch (handle.backing.type) {
                  case 'direct_blob':
                    return {
                      type: 'direct_blob',
                      blob: handle.backing.blob,
                    };
                  case 'reader_only':
                    return {
                      type: 'stream',
                  ...
                  default: {
                    ...
                    throw new Error(`Unhandled binary object backing: ${((_ex satisfies never) as { readonly type: string }).type}`);
                  }
                  }
                })();
                yield {
        ...
        console.warn('[PlainOPFSStorageBackend] Failed to dump some binary objects', e);
      }
    };
  ...
  async *listVolumes(): AsyncIterable<Volume> {
    ...
    try {
      ...
      console.error('[PlainOPFSStorageBackend] Failed to list volumes', e);
    }
  }
    ...
    try {
      ...
      case 'opfs': {
        ...
        return {
          ...
          handle: await shardDir.getDirectoryHandle(idToRaw({ id: volumeId })),
        };
      }
      case 'host': {
  ...
  async importVolumeForTransition({
    ...
    case 'opfs': {
      ...
      await copyStorageDirectory({
        ...
        target: createDirectStorageDirectoryTransferTarget({ root: targetDirectory }),
        signal,
      });
      break;
    }
  ...
  async openSpecialFileSystemForTransition({
    ...
    try {
      return {
        type: 'direct_directory',
        handle: await root.getDirectoryHandle(name, { create }),
      };
    } catch (error) {
      const notFound = error instanceof DOMException
        ? error.name === 'NotFoundError'
  ...
  async openSpecialFileSystemDirectory({
    ...
    const rootDirectory = (() => {
      ...
      case 'encrypted_directory':
        throw new Error('Plain OPFS special filesystem returned encrypted directory access');
      default: {
  ...
  async removeSpecialFileSystemEntry({
    ...
    try {
      await parent.removeEntry(segments.at(-1)!, { recursive });
    } catch (error) {
      const notFound = error instanceof DOMException
        ? error.name === 'NotFoundError'
        : error instanceof Error && error.message.startsWith('NotFoundError');
  ...
  async removeSpecialFileSystemForTransition({
    ...
    const root = await navigator.storage.getDirectory();
    try {
      await root.removeEntry(this.getSpecialFileSystemName({ type }), { recursive: true });
    } catch (error) {
      const notFound = error instanceof DOMException
        ? error.name === 'NotFoundError'
        : error instanceof Error && error.message.startsWith('NotFoundError');
