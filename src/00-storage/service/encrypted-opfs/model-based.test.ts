...
const MODEL_POLICY: EncryptedOpfsPolicy = {
...
async function runModelScenario({ seed }: {
  ...
  let session = await TEST_ONLY.createEncryptedOpfsInternal({
    backingDirectory,
  ...
  for (let step = 0; step < 90; step += 1) {
    ...
    if (step > 0 && step % 17 === 0) {
      await session.close();
      session = await TEST_ONLY.openEncryptedOpfsInternal({
        backingDirectory,
        fileSystemRootKey: ROOT_KEY,
        policy: MODEL_POLICY,
        now,
  ...
  await session.close();
  const reopened = await TEST_ONLY.openEncryptedOpfsInternal({
    backingDirectory,
    fileSystemRootKey: ROOT_KEY,
    policy: MODEL_POLICY,
    now,
...
describe('EncryptedOpfs model-based file-system behavior', () => {
