...
describe('EncryptedOpfs public file-system API', () => {
  ...
  it('round-trips an inline file across a complete reopen', async () => {
    ...
    const first = await createEncryptedOpfs({
      backingDirectory: backing,
      fileSystemRootKey: ROOT_KEY,
    });
    ...
    const second = await openEncryptedOpfs({
      backingDirectory: backing,
      fileSystemRootKey: ROOT_KEY,
    });
  ...
  it('fails closed when opened with the wrong root key', async () => {
    const backing = new MockFileSystemDirectoryHandle({ name: 'backing' });
    await createEncryptedOpfs({ backingDirectory: backing, fileSystemRootKey: ROOT_KEY });
    await expect(openEncryptedOpfs({
      backingDirectory: backing,
  ...
  it('uses the persisted chunk size when a later implementation policy changes', async () => {
    ...
    const changedPolicy: EncryptedOpfsPolicy = {
      ...TINY_POLICY,
      fileChunkSize: 7,
    };
    const second = await TEST_ONLY.openEncryptedOpfsInternal({
      backingDirectory: backing,
      fileSystemRootKey: ROOT_KEY,
