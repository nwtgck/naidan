...
import type { LoadedEncryptedOpfsFile } from './node-service';
...
function assertReadArguments({
  ...
  for (const [fieldName, value] of [
    ...
    if (!Number.isSafeInteger(value) || value < 0) {
      throw new Error(`EncryptedOpfs read ${fieldName} must be a non-negative safe integer`);
    }
  }
  if (offset + length > buffer.byteLength) {
    throw new Error('EncryptedOpfs read range exceeds the destination buffer');
  }
}

export class EncryptedOpfsFileReader implements StorageBinaryObjectReadHandle {
  constructor({
    file,
    extentIndex,
    chunkStore,
  ...
  private readonly file: LoadedEncryptedOpfsFile;
  ...
  stream({ start, end, signal }: {
    ...
    if (!Number.isSafeInteger(start) || start < 0) {
      throw new Error('EncryptedOpfs stream start must be a non-negative safe integer');
    }
    if (end !== undefined && (!Number.isSafeInteger(end) || end < start)) {
      throw new Error('EncryptedOpfs stream end must be a safe integer not smaller than start');
    }
  ...
  private async readExtents({ buffer, offset, length, position, signal }: {
    ...
    case 'extents':
      break;
    case 'inline':
      throw new Error('EncryptedOpfs extent reader received an inline file');
    default: {
  ...
  private assertOpen(): void {
    if (this.closed) {
      throw new Error('EncryptedOpfs file reader is closed');
    }
  }
}
