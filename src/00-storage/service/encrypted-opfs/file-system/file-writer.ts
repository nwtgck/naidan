...
import type { EncryptedOpfsCore } from './core';
import type { EncryptedOpfsExtentIndex } from './extent-index';
import type { EncryptedOpfsFileChunkStore } from './file-chunk-store';
import type { EncryptedOpfsInodeStore } from './inode-store';
import type {
  EncryptedOpfsNodeService,
  LoadedEncryptedOpfsFile,
} from './node-service';
...
export class EncryptedOpfsFileWriter implements StorageWritableFile {
  constructor({
    core,
    nodeService,
    inodeStore,
    ...
    core: EncryptedOpfsCore;
    nodeService: EncryptedOpfsNodeService;
    inodeStore: EncryptedOpfsInodeStore;
    extentIndex: EncryptedOpfsExtentIndex;
    chunkStore: EncryptedOpfsFileChunkStore;
    policy: EncryptedOpfsPolicy;
    baseFile: LoadedEncryptedOpfsFile;
  ...
  private readonly core: EncryptedOpfsCore;
  private readonly nodeService: EncryptedOpfsNodeService;
  private readonly inodeStore: EncryptedOpfsInodeStore;
  private readonly extentIndex: EncryptedOpfsExtentIndex;
  private readonly chunkStore: EncryptedOpfsFileChunkStore;
  private readonly policy: EncryptedOpfsPolicy;
  private readonly baseFile: LoadedEncryptedOpfsFile;
  ...
  async write({ position, data }: {
    ...
    if (position + data.byteLength > Number.MAX_SAFE_INTEGER) {
      throw new Error('EncryptedOpfs write end exceeds the safe integer range');
    }
  ...
  async close(): Promise<void> {
    ...
    try {
      ...
      default: {
        ...
        throw new Error(`Unhandled EncryptedOpfs writer dirty state: ${String(_ex)}`);
      }
      }
      ...
      await this.core.mutate({
        operation: async ({ state }) => {
          ...
          if (currentFile.inode.revision !== this.baseFile.inode.revision) {
            throw new Error('EncryptedOpfs file changed while its writer was open');
          }

          const modifiedAt = this.now();
          let nextInode: EncryptedOpfsFileInodeDto;
  ...
  private assertOpen(): void {
    if (this.settled) {
      throw new Error('EncryptedOpfs file writer is already closed or aborted');
    }
  }
}
