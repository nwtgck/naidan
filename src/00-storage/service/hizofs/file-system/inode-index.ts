...
function assertPage({ page }: {
  ...
  case 'leaf': {
    ...
    for (const entry of page.entries) {
      ...
      if (
        ...
        && compareHizoFSStrings({ left: previousNodeId, right: entry.nodeId }) >= 0
      ) {
        throw new Error('HizoFS inode index leaf entries must be strictly sorted');
      }
  ...
  case 'branch': {
    if (page.children.length === 0) {
      throw new Error('HizoFS inode index branch must contain at least one child');
    }
    ...
    for (const child of page.children) {
      ...
      if (
        ...
        throw new Error('HizoFS inode index branch bounds must be strictly sorted');
      }
