...
<template>
  <div class="mt-2 pt-4 border-t border-gray-50 dark:border-gray-800/50">
    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 opacity-70">
      {{ strings.ProviderProfilePreview__configuration_preview() }}
    </p>

    <div class="space-y-2 px-1">
      <!-- Provider & Model -->
      <div class="flex items-center justify-between text-[11px]">
        <span class="text-gray-400 font-medium">{{ strings.ProviderProfilePreview__provider_and_model() }}</span>
        <span class="font-bold text-gray-500 dark:text-gray-400">
          {{ capitalize({ value: form.endpointType }) }} / {{ form.defaultModelId || strings.ProviderProfilePreview__none() }}
        </span>
      </div>

      <!-- Endpoint URL -->
      <div class="flex items-center justify-between text-[11px]">
        <span class="text-gray-400 font-medium">{{ strings.ProviderProfilePreview__endpoint_url() }}</span>
      ...
      <div v-if="form.endpointHttpHeaders?.length || form.systemPrompt || form.lmParameters"
        ...
        <span v-if="form.endpointHttpHeaders?.length"
          ...
          {{ strings.ProviderProfilePreview__headers() }}
        </span>
        <span v-if="form.systemPrompt"
              class="text-[9px] font-bold text-gray-400 flex items-center gap-1">
          <span class="w-1 h-1 rounded-full bg-gray-300 dark:bg-gray-600"></span>
          {{ strings.ProviderProfilePreview__system_prompt() }}
        </span>
        <span v-if="form.lmParameters"
              class="text-[9px] font-bold text-gray-400 flex items-center gap-1">
          <span class="w-1 h-1 rounded-full bg-gray-300 dark:bg-gray-600"></span>
          {{ strings.ProviderProfilePreview__lm_params() }}
        </span>
      </div>
    </div>
  </div>
