...
<template>
  <div tw-class="mt-2 pt-4 border-t border-gray-50 dark:border-gray-800/50">
    <p tw-class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 opacity-70">
      {{ lazyStrings.ProviderProfilePreview__configuration_preview() }}
    </p>

    <div tw-class="space-y-2 px-1">
      <!-- Provider & Model -->
      ...
      <!-- Endpoint URL -->
      <div tw-class="flex items-center justify-between text-[11px]">
        <span tw-class="text-gray-400 font-medium">{{ lazyStrings.ProviderProfilePreview__endpoint_url() }}</span>
        <span tw-class="font-bold text-gray-500 dark:text-gray-400 truncate max-w-[180px]">
      ...
      <div v-if="(isHttpEndpoint(form.endpoint) && form.endpoint.httpHeaders?.length) || form.systemPrompt || form.lmParameters"
           tw-class="flex items-center gap-2 pt-1">
        <span v-if="isHttpEndpoint(form.endpoint) && form.endpoint.httpHeaders?.length"
              tw-class="text-[9px] font-bold text-gray-400 flex items-center gap-1">
          <span tw-class="w-1 h-1 rounded-full bg-gray-300 dark:bg-gray-600"></span>
