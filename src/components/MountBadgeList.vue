...
<template>
  <div tw-class="flex flex-wrap gap-2" data-testid="mount-badge-list">
    <div
      v-for="mount in mounts"
      :key="idToRaw({ id: mount.volumeId })"
      tw-class="flex items-center gap-1 pl-2 pr-1 py-1 rounded-lg bg-blue-50 dark:bg-blue-950/50 border border-blue-100 dark:border-blue-900 text-blue-700 dark:text-blue-300 text-xs font-medium"
      data-testid="mount-badge"
    >
      <FolderIcon tw-class="w-3.5 h-3.5 shrink-0" />
      <button
        v-if="showExplorer"
        tw-class="max-w-[120px] truncate mx-1 hover:underline focus:outline-none"
      ...
      <span
        v-else
        tw-class="max-w-[140px] truncate mx-1"
      ...
      <button
        ...
        tw-class="p-0.5 rounded hover:bg-blue-100 dark:hover:bg-blue-900 transition-colors"
        data-testid="mount-toggle-readonly"
      >
        <LockIcon v-if="mount.readOnly" tw-class="w-3 h-3 text-green-500 dark:text-green-400" />
        <UnlockIcon v-else tw-class="w-3 h-3 text-amber-500 dark:text-amber-400" />
      </button>
      <button
        @click="emit('remove', { volumeId: mount.volumeId })"
        :title="lazyStrings.MountBadgeList__remove()"
        tw-class="p-0.5 rounded hover:bg-blue-100 dark:hover:bg-blue-900 transition-colors text-blue-400 hover:text-red-500 dark:hover:text-red-400"
