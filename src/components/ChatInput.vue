...
async function attachCopyAsVolume({ entries, name }: {
  ...
  try {
    ...
    if ((e as Error).name !== 'AbortError') {
      addToast({ message: await stringsAsync.ChatInput__failed_to_copy({
        name,
...
async function attachLinkAsVolume() {
  ...
  try {
    ...
    if ((e as Error).name !== 'AbortError') {
      addToast({ message: await stringsAsync.ChatInput__failed_to_link_folder({
        errorMessage: (e as Error).message,
      }) });
    }
  }
  ...
  const confirmCopy = await (async () => {
    ...
    case 'host': {
      const [title, message, confirmButtonText] = await Promise.all([
        stringsAsync.ChatInput__unlink_folder(),
        stringsAsync.ChatInput__stop_using_this_folder_in_this_chat_your_original_files_on_disk_stay_safe_and_intact(),
        stringsAsync.ChatInput__unlink(),
      ]);
    ...
    case undefined: {
      const [title, message, confirmButtonText] = await Promise.all([
        stringsAsync.ChatInput__remove_folder(),
        stringsAsync.ChatInput__remove_the_copied_folder_from_this_chat_the_copy_stored_in_the_browser_will_be_deleted(),
        stringsAsync.ChatInput__remove(),
      ]);
      return { title, message, confirmButtonText };
    }
    default: {
  ...
  >
    ...
    >
      ...
      <textarea
        ...
        :placeholder="strings.ChatInput__type_a_message()"
