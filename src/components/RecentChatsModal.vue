...
<template>
  <Transition name="fade">
    <div v-if="isRecentOpen" tw-class="fixed inset-0 z-50 flex items-start justify-center pt-[10vh] px-4" @click.self="closeRecent">
      <div tw-class="absolute inset-0 bg-black/20 dark:bg-black/50 backdrop-blur-sm transition-opacity" aria-hidden="true" @click="closeRecent" />

      <div tw-class="relative w-full max-w-5xl bg-white dark:bg-gray-900 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-800 flex flex-col h-[70vh] overflow-hidden transform transition-all scale-100 outline-none" @click.stop @keydown="event => handleKeydown({ event })" tabindex="-1">
        ...
        <div tw-class="flex items-center gap-3 p-4 border-b border-gray-100 dark:border-gray-800 shrink-0">
          ...
          />
          <button @click="closeRecent" tw-class="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
          ...
          <div v-if="isPreviewVisible && recentChats.length > 0"
            ...
            <div v-else tw-class="h-full flex items-center justify-center bg-gray-50/50 dark:bg-gray-950/20">
              <div tw-class="flex flex-col items-center gap-2 opacity-20">
                ...
                <span tw-class="text-[10px] font-black uppercase tracking-[0.3em] text-gray-500">{{ lazyStrings.RecentChatsModal__peek() }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div tw-class="p-2.5 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-950/50 text-[10px] font-bold text-gray-400 flex justify-between px-6 shrink-0">
          <div tw-class="flex gap-6">
            <span tw-class="flex items-center gap-1.5"><kbd tw-class="px-1.5 py-0.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded text-gray-600 dark:text-gray-400 font-sans">↑↓</kbd> {{ lazyStrings.RecentChatsModal__navigate() }}</span>
            <span tw-class="flex items-center gap-1.5"><kbd tw-class="px-1.5 py-0.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded text-gray-600 dark:text-gray-400 font-sans">↵</kbd> {{ lazyStrings.RecentChatsModal__select() }}</span>
            <span tw-class="flex items-center gap-1.5"><kbd tw-class="px-1.5 py-0.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded text-gray-600 dark:text-gray-400 font-sans">→</kbd> {{ lazyStrings.RecentChatsModal__preview() }}</span>
          </div>
