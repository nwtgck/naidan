...
const props = defineProps<{ isOpen: boolean; chatMounts: readonly Mount[] | undefined }>();
...
watch(() => props.isOpen, async (open) => {
  if (!open) return;
  await reopenSessionIfNeeded({ chatMounts: props.chatMounts ?? [] });
  await nextTick();
...
async function handleCreateSession() {
  await createChatWorkerSession({ chatMounts: props.chatMounts ?? [] });
  await nextTick();
  inputRef.value?.focus();
}
      ...
      <div class="w-full max-w-[1100px] h-[92vh] bg-white dark:bg-gray-900 rounded-3xl shadow-2xl overflow-hidden border border-gray-200 dark:border-gray-800 flex flex-col">
        <header class="flex items-center justify-between gap-4 px-4 md:px-5 py-3 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900">
          <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="min-w-0">
              <h2 class="text-sm md:text-base font-bold text-gray-900 dark:text-white tracking-tight">Wesh Terminal</h2>
              <p class="text-[11px] md:text-xs text-gray-500 dark:text-gray-400">Worker-backed terminal with chat volumes mounted.</p>
            </div>
          </div>
          <button
            @click="emit('close')"
