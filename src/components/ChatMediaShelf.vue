...
<template>
  <div class="animate-in slide-in-from-bottom-full" tw-class="bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-t border-gray-100 dark:border-gray-800 shadow-2xl z-40 flex flex-col h-[70vh] min-h-[400px] overflow-visible duration-300">
    <!-- Header -->
    <div tw-class="px-4 py-2 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between shrink-0">
      <div tw-class="flex items-center gap-2">
        <div tw-class="p-1 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          <ImageIcon tw-class="w-4 h-4 text-blue-500" />
        </div>
        <span tw-class="text-xs font-bold text-gray-700 dark:text-gray-200 uppercase tracking-widest">{{ lazyStrings.ChatMediaShelf__media_shelf() }}</span>
        <span tw-class="text-[10px] font-bold px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-500 rounded-full">
          {{ allMediaItems.length }}
        </span>
        <div tw-class="w-px h-3 bg-gray-200 dark:bg-gray-700 mx-1"></div>
        <button
          @click="mediaOrder = mediaOrder === 'forward' ? 'reverse' : 'forward'"
          :tw-class="['flex items-center gap-1.5 px-2 py-0.5 rounded-lg text-[10px] font-bold transition-all', mediaOrder === 'reverse'
        ...
        >
          <SortDescIcon v-if="mediaOrder === 'forward'" tw-class="w-3 h-3" />
          <SortAscIcon v-else tw-class="w-3 h-3" />
          <span tw-class="uppercase tracking-wider">{{ mediaOrder === 'forward' ? lazyStrings.ChatMediaShelf__forward() : lazyStrings.ChatMediaShelf__reverse() }}</span>
        </button>
      </div>
      <button
        @click="emit('close')"
        tw-class="p-1 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-400 hover:text-red-500 transition-colors"
    ...
    <div
      ref="scrollContainer"
      tw-class="flex-1 overflow-y-auto px-4 py-2 space-y-4"
    >
      <div v-if="mediaGroups.length === 0" tw-class="h-full flex flex-col items-center justify-center text-gray-400 dark:text-gray-500 gap-2">
        <ImageIcon tw-class="w-12 h-12 opacity-10" />
        <span tw-class="text-sm italic">{{ lazyStrings.ChatMediaShelf__no_images_in_this_chat_yet() }}</span>
      </div>
      <template v-else>
        <div
          v-for="group in mediaGroups"
          :key="idToRaw({ id: group.messageId })"
          tw-class="flex flex-col gap-3 group/group relative hover:z-30"
        >
          <!-- Message Header: Jump Button & Shared Prompt -->
          <div tw-class="flex items-start justify-between gap-4">
            <div tw-class="flex-1 min-w-0">
              <div tw-class="flex items-center gap-2 mb-1">
                <button
                  @click="emit('jump-to-message', group.messageId)"
                  tw-class="flex items-center gap-1 px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-800 text-[9px] font-bold text-gray-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-all uppercase tracking-widest shrink-0"
                  :title="lazyStrings.ChatMediaShelf__jump_to_this_message_in_chat()"
                >
                  <ExternalLinkIcon tw-class="w-2.5 h-2.5" />
                  <span>{{ lazyStrings.ChatMediaShelf__jump() }}</span>
                </button>

                <div
                  v-if="group.prompt"
                  tw-class="group/prompt relative flex-1 min-w-0 cursor-pointer"
                  @click="copyPrompt({ prompt: group.prompt, messageId: group.messageId })"
                  :title="lazyStrings.ChatMediaShelf__click_to_copy_prompt()"
                >
                  <div tw-class="text-[10px] font-bold text-gray-400 dark:text-gray-500 truncate italic hover:text-blue-500 transition-colors pr-8">
                    {{ group.prompt }}
                  </div>
                  <div tw-class="absolute right-0 top-1/2 -translate-y-1/2 opacity-0 group-hover/prompt:opacity-100 transition-opacity flex items-center gap-1 bg-white/80 dark:bg-gray-900/80 px-1 rounded shadow-sm">
                    <template v-if="copiedPromptId === group.messageId">
                      <CheckIcon tw-class="w-2.5 h-2.5 text-green-500" />
                      <span tw-class="text-[8px] font-bold text-green-500 uppercase">{{ lazyStrings.ChatMediaShelf__copied() }}</span>
                    </template>
                    <template v-else>
                      <CopyIcon tw-class="w-2.5 h-2.5 text-blue-500" />
                    </template>
                  </div>
                </div>
                <div v-else tw-class="text-[10px] font-bold text-gray-300 dark:text-gray-700 uppercase tracking-widest italic">
          ...
          <div class="no-scrollbar" tw-class="flex items-center gap-4 overflow-x-auto pb-32 -mb-28 -mx-1 px-1">
            <div
            ...
            >
              <div tw-class="absolute inset-0 rounded-2xl border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 shadow-sm hover:shadow-md hover:border-blue-500/50 transition-all overflow-hidden">
                <img
                  ...
                  tw-class="w-full h-full object-cover transition-transform group-hover/item:scale-110"
                />
                <div v-else tw-class="w-full h-full flex items-center justify-center">
                  <ImageIcon tw-class="w-8 h-8 text-gray-300 animate-pulse" />
                </div>

                <!-- Hover Gradient -->
                <div tw-class="absolute inset-0 bg-gradient-to-t from-black/20 via-transparent to-transparent opacity-0 group-hover/item:opacity-100 transition-opacity pointer-events-none"></div>
              </div>

              <!-- Item Overlay - Top Buttons -->
              <div tw-class="absolute top-2 right-2 z-50 flex flex-col gap-1.5 opacity-0 group-hover/item:opacity-100 transition-all origin-top-right overflow-visible">
                <div @click.stop>
                  <ImageDownloadButton
                ...
                <button
                  ...
                  tw-class="p-2 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm border border-gray-100 dark:border-gray-700 rounded-lg text-gray-500 hover:text-blue-600 shadow-sm transition-colors flex items-center justify-center"
              ...
              <div
                ...
                class="animate-in fade-in zoom-in" tw-class="absolute inset-0 z-50 bg-white/95 dark:bg-gray-900/95 p-3 rounded-2xl border border-blue-200 dark:border-blue-800 flex flex-col gap-2 duration-200 shadow-xl"
                @click.stop
              >
                <div tw-class="flex items-center justify-between border-b dark:border-gray-800 pb-1.5 mb-1">
                  <span tw-class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{{ lazyStrings.ChatMediaShelf__parameters() }}</span>
                  <button @click="showingInfoId = null" tw-class="p-1 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg text-gray-400 hover:text-red-500 transition-colors">
                    <XIcon tw-class="w-3.5 h-3.5" />
                  </button>
                </div>

                <div class="no-scrollbar" tw-class="space-y-1.5 flex-1 overflow-y-auto pr-1">
                  <!-- Steps -->
                  <div tw-class="flex items-center justify-between group/field">
                    <div tw-class="flex items-center gap-1.5 text-[9px] font-bold text-gray-500">
                      <ZapIcon tw-class="w-2.5 h-2.5" />
                      <span>{{ lazyStrings.ChatMediaShelf__steps() }}</span>
                    </div>
                    <button
                      @click="copyField({ text: item.steps, field: 'steps' })"
                      tw-class="flex items-center gap-1.5 px-1.5 py-0.5 rounded hover:bg-blue-50 dark:hover:bg-blue-900/30 text-[10px] font-mono text-gray-700 dark:text-gray-300 transition-all"
                    >
                      <span>{{ item.steps ?? lazyStrings.ChatMediaShelf__not_available() }}</span>
                      <template v-if="copiedField === `${item.id}-steps`">
                        <CheckIcon tw-class="w-2.5 h-2.5 text-green-500" />
                      </template>
                      <CopyIcon v-else tw-class="w-2.5 h-2.5 opacity-0 group-hover/field:opacity-100 text-blue-500" />
                    </button>
                  </div>
                  ...
                  <div tw-class="flex items-center justify-between group/field">
                    ...
                    <button
                      ...
                      tw-class="flex items-center gap-1.5 px-1.5 py-0.5 rounded hover:bg-blue-50 dark:hover:bg-blue-900/30 text-[10px] font-mono text-gray-700 dark:text-gray-300 transition-all"
                    >
                      <span tw-class="truncate max-w-[60px]">{{ item.seed ?? lazyStrings.ChatMediaShelf__not_available() }}</span>
                  ...
                  <!-- Model -->
                  <div tw-class="flex flex-col gap-1 group/field">
                    <div tw-class="flex items-center gap-1.5 text-[9px] font-bold text-gray-500">
                    ...
                    <button
                      ...
                      tw-class="flex items-center justify-between w-full px-1.5 py-0.5 rounded hover:bg-blue-50 dark:hover:bg-blue-900/30 text-[9px] font-mono text-gray-700 dark:text-gray-300 transition-all text-left"
                    >
                      <span tw-class="truncate">{{ item.model ?? lazyStrings.ChatMediaShelf__not_available() }}</span>
                      <template v-if="copiedField === `${item.id}-model`">
                        <CheckIcon tw-class="w-2.5 h-2.5 text-green-500 shrink-0" />
                      </template>
                      <CopyIcon v-else tw-class="w-2.5 h-2.5 opacity-0 group-hover/field:opacity-100 text-blue-500 shrink-0" />
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div tw-class="absolute bottom-2 left-2 z-20">
                <div tw-class="px-1.5 py-0.5 bg-black/50 backdrop-blur-md rounded text-[9px] font-bold text-white shadow-sm border border-white/10">
