...
const dragHoverGroup = ref<ChatGroupId | null>(null);
let dragHoverTimeout: ReturnType<typeof setTimeout> | null = null;
let isInternalUpdate = false;

const editingId = ref<ChatId | null>(null);
const editingTitle = ref('');
const isCreatingChatGroup = ref(false);
const newChatGroupName = ref('');
const editingChatGroupId = ref<ChatGroupId | null>(null);
const editingChatGroupName = ref('');
const skipLeaveAnimation = ref(false);
const lastNavigatedId = ref<string | null>(null);

const activeActionGroupId = ref<ChatGroupId | null>(null);

const COMPACT_THRESHOLD = 5;
const expandedGroupIds = ref<Set<ChatGroupId>>(new Set());
const collapsingGroupIds = ref<Set<ChatGroupId>>(new Set());
...
const visibleItems = computed(() => {
  ...
  collect({ list: sidebarItemsLocal.value });
  return result;
});
...
function isGroupCompactExpanded({ groupId }: { groupId: ChatGroupId }) {
  return expandedGroupIds.value.has(groupId);
}

function toggleGroupCompactExpansion({ groupId }: { groupId: ChatGroupId }) {
...
function onDragOverGroup({ groupId }: { groupId: ChatGroupId }) {
...
async function handleDeleteChatGroup({ group }: { group: ChatGroup }) {
...
async function handleNewChat({ groupId }: { groupId: ChatGroupId | undefined }) {
...
function startEditingChatGroup({ chatGroup }: { chatGroup: ChatGroup }) {
...
async function handleGlobalModelChange({ newModelId }: { newModelId: string | undefined }) {
  if (!newModelId) return;
  await updateGlobalModel({ modelId: newModelId });
}

function getGroupItems({ groupId }: { groupId: ChatGroupId }) {
...
function handleToggleChatGroupCollapse({ chatGroup }: { chatGroup: ChatGroup }) {
  ...
  if (e.key === 'ArrowDown') {
    ...
    if (nextIndex < visibleItems.value.length) {
      ...
      if (item) {
        ...
        switch (type) {
        case 'chat':
          handleOpenChat({ id: item.id });
          break;
        case 'chat_group':
          handleOpenChatGroup({ id: item.id });
          break;
        case 'expand_button':
          break;
        default: {
    ...
    if (currentItem?.type === 'expand_button' && currentItem.groupId) {
      if (e.key === 'ArrowRight' && !isGroupCompactExpanded({ groupId: currentItem.groupId })) {
        e.preventDefault();
        toggleGroupCompactExpansion({ groupId: currentItem.groupId });
      ...
      } else if (e.key === 'ArrowLeft' && isGroupCompactExpanded({ groupId: currentItem.groupId })) {
        e.preventDefault();
        toggleGroupCompactExpansion({ groupId: currentItem.groupId });
      }
      return;
    ...
    if (groupId) {
      ...
      if (group) {
        if (e.key === 'ArrowRight' && group.isCollapsed) {
          e.preventDefault();
          handleToggleChatGroupCollapse({ chatGroup: group });
...
<template>
  <div tw-class="flex flex-col h-full bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 select-none transition-colors">
              ...
              <div v-if="element.type === 'chat_group'" tw-class="space-y-1">
                ...
                <div tw-class="grid transition-all duration-200 ease-in-out" :style="{ gridTemplateRows: element.chatGroup.isCollapsed ? '0fr' : '1fr' }">
                  <div tw-class="ml-4 pl-2 border-l border-gray-100 dark:border-gray-800 mt-1 space-y-0.5 overflow-hidden min-h-0">
                    ...
                    >
                      <draggable
                        v-model="useGroupItemsModel({ groupId: element.chatGroup.id }).value"
                        :group="{ name: 'sidebar' }"
                        item-key="id"
                        handle=".handle"
                        :animation="0"
    ...
    <div :tw-class="['border-t border-gray-100 dark:border-gray-800 space-y-3 bg-gray-50/30 dark:bg-black/20', isSidebarOpen ? 'p-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))]' : 'py-2 px-1 pb-[calc(0.5rem+env(safe-area-inset-bottom))]']">
      ...
      <div v-if="isSidebarOpen" class="animate-in fade-in" tw-class="px-1 space-y-2 duration-300">
        ...
        <ModelSelector
          ...
          :placeholder="lazyStrings.Sidebar__none()"
        />
      </div>
