...
async function handleDeleteProviderProfile({ id }: { id: ProviderProfileId }) {
  ...
  addToast({
    message: await stringsAsync.ProviderProfilesTab__profile_was_deleted({ profileName: deletedProfile.name }),
    actionLabel: await stringsAsync.ProviderProfilesTab__undo(),
    onAction: async () => {
  ...
  <div data-testid="profiles-section" class="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-400">
    <section class="space-y-6">
      <div class="flex items-center gap-2 pb-3 border-b border-gray-100 dark:border-gray-800">
        ...
        <h2 class="text-lg font-bold text-gray-800 dark:text-white tracking-tight">{{ strings.ProviderProfilesTab__provider_profiles() }}</h2>
      </div>

      <p class="text-sm font-medium text-gray-500">{{ strings.ProviderProfilesTab__save_and_switch_between_different_ai_provider_configurations_easily() }}</p>
      ...
      <div v-if="!profiles || profiles.length === 0" class="flex flex-col items-center justify-center p-16 bg-gray-50 dark:bg-gray-800/30 rounded-3xl border-2 border-dashed border-gray-200 dark:border-gray-800">
        ...
        <p class="text-sm text-gray-400 font-bold mb-6">{{ strings.ProviderProfilesTab__no_profiles_saved_yet() }}</p>
        <button
        ...
        >
          {{ strings.ProviderProfilesTab__go_to_connection_to_create_one() }}
        </button>
      </div>
        ...
        >
          <div class="flex-1 min-w-0 mr-6">
            ...
            <div class="text-[11px] font-bold text-gray-500 mt-2 flex items-center gap-3">
              <span class="flex items-center gap-1.5 bg-gray-50 dark:bg-gray-800 px-2 py-0.5 rounded-lg border border-gray-100 dark:border-gray-700">{{ providerProfile.defaultModelId || strings.ProviderProfilesTab__no_default_model() }}</span>
              <span v-if="providerProfile.titleModelId" class="text-[9px] opacity-60 px-2 py-0.5 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-transparent">{{ strings.ProviderProfilesTab__title_model({ modelId: providerProfile.titleModelId }) }}</span>
            </div>
          </div>
          ...
          <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-200">
            <button
              ...
              :title="strings.ProviderProfilesTab__rename_profile()"
            ...
            <button
              ...
              :title="strings.ProviderProfilesTab__delete_profile()"
