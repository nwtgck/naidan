      ...
      <div class="flex flex-col gap-3">
        ...
        <div class="flex flex-col gap-1">
          <div class="flex items-center justify-between">
            <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{{ lazyStrings.ImageInfoDisplay__prompt() }}</span>
            <button @click="copyPrompt({ event: $event })" class="text-gray-400 hover:text-blue-500 transition-colors" :title="lazyStrings.ImageInfoDisplay__copy_prompt()">
        ...
        <div class="grid grid-cols-2 gap-2 pt-2 border-t border-gray-100 dark:border-gray-700">
          <div v-if="width && height" class="flex flex-col">
            <span class="text-[9px] font-bold text-gray-400 uppercase tracking-widest">{{ lazyStrings.ImageInfoDisplay__size() }}</span>
          ...
          <div v-if="steps !== undefined" class="flex flex-col">
            <span class="text-[9px] font-bold text-gray-400 uppercase tracking-widest">{{ lazyStrings.ImageInfoDisplay__steps() }}</span>
          ...
          <div v-if="seed !== undefined" class="flex flex-col col-span-2">
            <div class="flex items-center justify-between">
              <span class="text-[9px] font-bold text-gray-400 uppercase tracking-widest">{{ lazyStrings.ImageInfoDisplay__seed() }}</span>
              <button @click="copySeed({ event: $event })" class="text-gray-400 hover:text-blue-500 transition-colors" :title="lazyStrings.ImageInfoDisplay__copy_seed()">
