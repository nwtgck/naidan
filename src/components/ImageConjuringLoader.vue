...
<template>
  <div tw-class="relative w-full max-w-[400px] aspect-square rounded-3xl overflow-hidden flex items-center justify-center group/conjure" data-testid="image-conjuring-loader">
    <!-- Ambient Glow & Grid -->
    <div tw-class="absolute inset-0 pointer-events-none">
    ...
    <!-- Magical Particles -->
    <div tw-class="absolute inset-0 overflow-hidden pointer-events-none">
    ...
    <!-- Central Core -->
    <div tw-class="relative flex flex-col items-center gap-6">
      <!-- Ambient glow -->
      <div tw-class="absolute w-40 h-40 bg-blue-500/15 blur-[50px] animate-pulse"></div>

      <div tw-class="relative z-10 flex flex-col items-center gap-4">
        <!-- Status Header -->
        <div tw-class="flex flex-col items-center gap-1">
          <span class="animate-subtle-pulse" tw-class="text-[11px] font-bold text-blue-500/80 dark:text-blue-400/80 drop-shadow-[0_0_8px_rgba(96,165,250,0.4)] whitespace-nowrap">
          ...
          <div v-if="displayedCurrentStep !== undefined" tw-class="flex flex-col items-center -space-y-1" data-testid="step-display">
            ...
            <span tw-class="text-[10px] font-mono font-bold text-blue-500/40 dark:text-blue-400/40 uppercase tracking-widest">{{ lazyStrings.ImageConjuringLoader__steps() }}</span>
          </div>
          <div v-else tw-class="h-14 flex items-center justify-center">
            <div tw-class="w-1.5 h-1.5 bg-blue-500/60 rounded-full animate-ping"></div>
          </div>
        </div>

        <!-- Progress bar for steps -->
        <div v-if="totalSteps" tw-class="w-32 h-1 bg-blue-500/10 dark:bg-blue-400/5 rounded-full overflow-hidden border border-blue-500/10">
          <div
            :key="currentNumber"
            class="animate-bar-reset" tw-class="h-full bg-gradient-to-r from-blue-600 to-blue-400 transition-all duration-500 ease-out relative"
            :style="{ width: `${stepProgress}%` }"
            data-testid="step-progress-bar"
          >
            <div class="animate-shimmer" tw-class="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent"></div>
          </div>
        </div>

        <!-- Image Count Indicators -->
        <div v-if="totalCount && totalCount > 1" tw-class="flex flex-col items-center gap-2 mt-1" data-testid="image-count-indicators">
          ...
          <span v-if="currentNumber !== undefined" tw-class="text-[10px] font-mono font-bold text-blue-400/60" data-testid="image-count-label">
