...
async function handleImportRecipes({ recipes }: { recipes: { newName: string, matchedModelId?: string, recipe: ChatGroupRecipe }[] }) {
  try {
    ...
    addToast({
      message: await stringsAsync.SettingsModal__successfully_imported_recipes_as_chat_groups({ recipeCount: recipes.length }),
      duration: 3000,
    });
  } catch (err) {
    console.error('Failed to import recipes:', err);
    addToast({
      message: await stringsAsync.SettingsModal__failed_to_import_recipes({ errorMessage: err instanceof Error ? err.message : String(err) }),
      duration: 5000,
    });
  }
}
...
async function handleCancel() {
  if (hasUnsavedConnectionChanges.value) {
    const [title, message, confirmButtonText, cancelButtonText] = await Promise.all([
      stringsAsync.SettingsModal__discard_unsaved_changes(),
      stringsAsync.SettingsModal__discard_unsaved_connection_changes(),
      stringsAsync.SettingsModal__discard(),
      stringsAsync.SettingsModal__keep_editing(),
    ]);
...
async function handleAnalyzeRecipes(): Promise<void> {
  ...
  for (const result of parseResults) {
    if (!result.success) {
      recipeAnalysisError.value = await stringsAsync.SettingsModal__parse_error({ errorMessage: result.error });
      continue;
    }

    const validation = ChatGroupRecipeSchema.safeParse(result.data);
    if (!validation.success) {
      recipeAnalysisError.value = await stringsAsync.SettingsModal__validation_error({ errorMessage: validation.error.message });
      continue;
    }
  ...
  if (newAnalyzed.length === 0 && !recipeAnalysisError.value) {
    recipeAnalysisError.value = await stringsAsync.SettingsModal__no_valid_recipes_found_in_input();
  }
      ...
      <div class="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-[95vw] h-[95vh] md:h-[90vh] overflow-hidden flex flex-col md:flex-row border border-gray-100 dark:border-gray-800 relative modal-content-zoom">
        ...
        <aside class="w-full md:w-72 flex-shrink-0 bg-gray-50/50 dark:bg-black/20 border-b md:border-b-0 md:border-r border-gray-100 dark:border-gray-800/50 flex flex-col min-h-0 transition-colors">
          ...
          <div class="p-4 md:p-6 border-b border-gray-100 dark:border-gray-800/50 flex items-center gap-3 shrink-0">
            ...
            <h2 class="text-base md:text-lg font-bold text-gray-800 dark:text-white tracking-tight">{{ strings.SettingsModal__settings() }}</h2>
          </div>
          ...
          <nav class="flex-1 overflow-x-auto md:overflow-y-auto p-3 md:p-4 flex md:flex-col gap-1.5 no-scrollbar min-h-0 overscroll-contain">
            ...
            >
              ...
              {{ strings.SettingsModal__connection() }}
            </button>
            <button
            ...
            >
              ...
              {{ strings.SettingsModal__provider_profiles() }}
            </button>
            <button
            ...
            >
              ...
              {{ strings.SettingsModal__transformers_js() }}
            </button>
            <button
            ...
            >
              ...
              {{ strings.SettingsModal__recipes() }}
            </button>
            <button
            ...
            >
              ...
              {{ strings.SettingsModal__storage() }}
            </button>
            <button
            ...
            >
              ...
              {{ strings.SettingsModal__folders() }}
            </button>
            <button
            ...
            >
              ...
              {{ strings.SettingsModal__developer() }}
            </button>
            <button
          ...
          <div class="p-3 md:p-4 border-t border-gray-100 dark:border-gray-800/50 mt-auto flex items-center gap-3">
            ...
            >
              ...
              <div class="text-[10px] md:text-xs font-bold text-green-800 dark:text-green-300 truncate">
                {{ strings.SettingsModal__standalone() }}
              </div>
            </a>
          </div>
        </aside>
