...
async function handleAnalyzeRecipes(): Promise<void> {
  ...
  for (const result of parseResults) {
    if (!result.success) {
      recipeAnalysisError.value = await stringsAsync.RecipeImportTab__parse_error({ errorMessage: result.error });
      continue;
    }

    const validation = ChatGroupRecipeSchema.safeParse(result.data);
    if (!validation.success) {
      recipeAnalysisError.value = await stringsAsync.RecipeImportTab__validation_error({ errorMessage: validation.error.message });
      continue;
    }
  ...
  if (newAnalyzed.length === 0 && !recipeAnalysisError.value) {
    recipeAnalysisError.value = await stringsAsync.RecipeImportTab__no_valid_recipes_found_in_input();
  }
  ...
  <div data-testid="recipes-section" class="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-400">
    <section class="space-y-6">
      <div class="flex items-center gap-2 pb-3 border-b border-gray-100 dark:border-gray-800">
        ...
        <h2 class="text-lg font-bold text-gray-800 dark:text-white tracking-tight">{{ strings.RecipeImportTab__recipes() }}</h2>
      </div>

      <p class="text-sm font-medium text-gray-500">
        {{ strings.RecipeImportTab__import_predefined_chat_group_settings_prompts_parameters_and_model_rules_from_json() }}
      </p>

      <div class="space-y-4">
        <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">{{ strings.RecipeImportTab__paste_recipe_json_concatenated_json_objects_supported() }}</label>
        <textarea
  ...
  "models": [{"type": "regex", "pattern": "claude-3", "flags": ["i"]}]
      ...
      <div v-if="analyzedRecipes.length > 0" class="space-y-6 pt-6 border-t border-gray-100 dark:border-gray-800 animate-in fade-in slide-in-from-top-2 duration-300">
        <div class="flex items-center justify-between">
          <h3 class="text-sm font-bold text-gray-400 uppercase tracking-widest ml-1">{{ strings.RecipeImportTab__detected_recipes({ recipeCount: analyzedRecipes.length }) }}</h3>
          <button
          ...
          >
            ...
            {{ strings.RecipeImportTab__import_selected() }}
          </button>
        </div>
          ...
          >
            ...
            <div class="flex-1 min-w-0 space-y-3">
              <div class="flex flex-col gap-1.5">
                <label class="text-[9px] font-bold text-gray-400 uppercase tracking-widest ml-0.5">{{ strings.RecipeImportTab__chat_group_name() }}</label>
                <input
                  ...
                  :placeholder="strings.RecipeImportTab__chat_group_name()"
                />
              ...
              <div class="space-y-2">
                <label class="text-[9px] font-bold text-gray-400 uppercase tracking-widest ml-0.5">{{ strings.RecipeImportTab__model_selection() }}</label>
                <div class="flex flex-col gap-2">
                  <ModelSelector
                    v-model="item.matchedModelId"
                    :models="getSortedModels({ matchedModelId: item.matchedModelId })"
                    :placeholder="strings.RecipeImportTab__use_default_model()"
                    allow-clear
                    :clear-label="strings.RecipeImportTab__use_default_model()"
                  />
              ...
              <div v-if="item.recipe.systemPrompt" class="p-3 bg-gray-50 dark:bg-black/20 rounded-xl border border-gray-100/50 dark:border-gray-800/50">
                <div class="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1">{{ strings.RecipeImportTab__system_prompt({ behavior: item.recipe.systemPrompt.behavior }) }}</div>
