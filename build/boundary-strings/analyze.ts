...
function analyzeScript({ lang, moduleId, sourceCode }: {
  ...
  const sourceFile = profileBuildSync({
    ...
    sample: { detail: moduleId, inputChars: sourceCode.length, items: 1 },
    run: () => parseTypeScript({
      moduleId,
  ...
  profileBuildSync({
    ...
    sample: { detail: moduleId, items: 1 },
...
function compileVueTemplate({ descriptor, moduleId }: {
  ...
  const result = profileBuildSync({
    ...
    sample: { detail: moduleId, inputChars: template.content.length, items: 1 },
    run: () => compileTemplate({
      compilerOptions: {
