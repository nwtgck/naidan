...
describe('collectBoundaryStringKeys', () => {
  it('collects direct synchronous and asynchronous string calls', () => {
    const sourceCode = `\
import {
  strings,
  stringsAsync,
...
<template>
  <textarea :placeholder="strings.ChatInput__type_a_message()" />
</template>
`;
  ...
  it('supports an imported alias for analysis while lint rejects it', () => {
    const sourceCode = `\
import { strings as localizedStrings } from '@/strings';
