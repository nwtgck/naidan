...
async function bundleView({ mode, entrySource, injectImageAsset }: {
  mode: 'standalone' | 'hosted', entrySource: string | undefined, injectImageAsset: boolean,
  ...
  const result = await build({
    ...
    resolve: { alias: [
      ...
      { find: /^@\/strings$/, replacement: 'virtual:image-strings' },
      { find: '@', replacement: path.resolve(root, 'src') },
    ] },
    plugins: [fixture, createStableDiffusionCppBrowserBuild({ rootDir: root, mode }),
