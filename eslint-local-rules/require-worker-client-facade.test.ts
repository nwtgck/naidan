...
describe('require-worker-client-facade rule', () => {
  ...
  it('reports relative worker client imports from non-facade runtime files', async () => {
    const messages = await lintText({
      filePath: path.resolve(repoRoot, 'src/features/highlight/worker/temp-client-shared.ts'),
    ...
    expect(messages[0]?.message).toContain('@/features/highlight/worker/client');
  });
  ...
  it('reports direct hosted client alias imports', async () => {
    const messages = await lintText({
      ...
      code: `import { createHighlightWorkerClient } from '@/features/highlight/worker/client-hosted'`,
    });
  ...
  it('reports dynamic imports of direct hosted clients', async () => {
    const messages = await lintText({
      filePath: path.resolve(repoRoot, 'src/components/TempWorkerClientConsumer.ts'),
      code: `const client = await import('@/features/highlight/worker/client-hosted')`,
    });
  ...
  it('allows facade files to re-export client-hosted', async () => {
    const messages = await lintText({
      filePath: path.resolve(repoRoot, 'src/features/highlight/worker/client.ts'),
  ...
  it('allows tests to import implementation files directly', async () => {
    const messages = await lintText({
      filePath: path.resolve(repoRoot, 'src/features/highlight/worker/client-hosted.test.ts'),
  ...
  it('reports relative imports that resolve to worker implementations across directories', async () => {
    const messages = await lintText({
      filePath: path.resolve(repoRoot, 'src/features/highlight/helpers/shared.ts'),
  ...
  it('reports nested worker implementation imports with the nested facade path', async () => {
    const messages = await lintText({
      filePath: path.resolve(repoRoot, 'src/features/transformers-js/scanner/worker/client-shared.ts'),
      code: `import { createTransformersJsScannerWorkerClient } from './client-standalone'`,
    });

    expect(messages).toHaveLength(1);
    expect(messages[0]?.message).toContain('@/features/transformers-js/scanner/worker/client');
  });

  it('ignores non-standalone worker-shaped imports outside the shared facade list', async () => {
    const messages = await lintText({
      filePath: path.resolve(repoRoot, 'src/features/example/worker/client-shared.ts'),
