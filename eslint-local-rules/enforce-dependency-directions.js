...
function createImportPathReporter({ context }) {
  ...
  function checkSourceNode({ sourceNode, reportNode }) {
    const importPath = getStringLiteralValue({ node: sourceNode });
...
function getMockSourceNode({ node }) {
  ...
  const isMockMember = callee.type === 'MemberExpression'
    && !callee.computed
  ...
  if (!isRequire && !isMockMember) {
    return undefined;
  }

  const [sourceNode] = node.arguments;
  return sourceNode?.type === 'Literal' ? sourceNode : undefined;
}
...
export const rule = {
  ...
  create(context) {
    ...
    return {
      ...
      CallExpression(node) {
        const sourceNode = getMockSourceNode({ node });
        if (sourceNode) {
